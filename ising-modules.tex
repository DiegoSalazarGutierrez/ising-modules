...
By \Cref{prp:5}(vii), we can define a diagonalizable operator $H^M \in \End(\gr^G(M))$ by setting $H^M(\alpha^p_M(m)) = \alpha^p_M(L^M_0m)$ for $p \in \mathbb{Z}$ and $m \in G^pM$.
...
\begin{proof}
  ...
  Therefore, by \Cref{prp:5}(v), $a^{i_1M}_{(-n_1 - 1)}m \in G^{\Delta_{a^{i_1}} + \Delta_m - h - p + n_1}M = G^{n - p}M$.
  ...
  We verify that we obtain an isomorphism $\gr_F(M) \xrightarrow{\sim} \gr^G(M)$ directly from the definitions (cf. \Cref{prp:3}).
\end{proof}
...
\begin{remark}
  \label{rmk:6}
  By construction and \Cref{rmk:5}, for each pivot $\lambda$ of $A^W_n$, we have $m^W_\lambda \in W$, $m^K_\lambda \in K$ and $\lm(m^K_\lambda) = m_\lambda$.
\end{remark}
...
By \Cref{lmm:9}, \Cref{rmk:6} and the fact that $K$ is a Poisson submodule (not just a submodule), we have
\begin{equation*}
K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  Pick $y \in K'$ such that $\lm(y) = m_\eta$ and some power product $p_\tau \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_\tau y)) = m_\lambda$ (this can be done because of \Cref{rmk:7} and the fact that $\lm$ is multiplicative) so we set $x = (L_{-2})^k_{(0)}(p_\tau y) \in K'$ to get $\lm(x) = m_\lambda$.
...
\begin{proof}
  ...
  By \Cref{rmk:8}, $m_{[\eta, 1, \dots, 1]} \mid m_\lambda$ where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:8}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $m_\eta \nmid m_\lambda$.
