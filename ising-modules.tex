...
In \Cref{sec:comb-argum}, we compute the series $p(t, q)$ related to $P$ using combinatorial arguments.
In \Cref{sec:comp-lead-monom}, we exhibit the partitions in $P$ using Wang's results in \cite{wang_rationality_1993} and explain how we use software to get the exceptional partitions.
In \Cref{sec:proofs-main-theorems}, we give formal proofs of the theorems about $L(1/2, 1/2)$.
In the appendices, we show the SageMath \cite{sagemath} program used compute a Poisson submodule related to the exceptional partitions appearing in $R$ and briefly the case $L(1/2, 1/16)$.
...
Let $V$ be an $\mathbb{N}$-graded vertex algebra, meaning it has a Hamiltonian $H \in \End(V)$ whose eigenvalues are natural numbers.
...
Write $Y^M(\omega, z) = \sum_{n \in \mathbb{Z}}L^M_nz^{-n - 1}$.
...
Define the \emph{character} and the \emph{refined character} of $\gr^G(M)$ (with respect to gradation \eqref{eq:1} and gradation \eqref{eq:2}) as
\begin{align*}
...
\begin{remark}
  \label{rmk:1}
  It is possible to prove that $\gr^G(M)$ and $\gr_F(M)$ are isomorphic modules.
  The proof is essentially the same as the proof of \cite[Proposition 2.6.1]{arakawa_remark_2012}.
\end{remark}

From now on, take $V = \Vir^{1/2}$ as the $\mathbb{N}$-graded vertex algebra with conformal vector $\omega = L_{-2}\vac$.
Some indexes or subindexes will not be written, so for example $\alpha^p_M$ simplifies to $\alpha^p$.
Set
\begin{align*}
  M &= M(1/2, 1/2), \\
  L &= L(1/2, 1/2), \\
...
For $V$, $\gr^G(V)$ is isomorphic to $\mathbb{C}[L_{-2}, L_{-3}, \dots]$ because $\gr^G(V) \cong \gr(U(\Vir_{\le -2})) \cong S(\Vir_{\le -2})$.
...
\begin{proof}
  ...
  \begin{equation*}
    \text{for }a \in U(\Vir)\{v_{3, 4}\}, u \in M\text{ and }n \in \mathbb{Z}, a_{(n)}u \in W,
  \end{equation*}
  ...
  \end{equation*}
  Let $\alpha^p(a) \in I$ with $a \in U(\Vir)\{v_{3, 4}\} \cap G^pV$.
...
\begin{remark}
  \label{rmk:3}
  By construction and \Cref{rmk:2}, for each pivot $\lambda$ of $A^W_n$, we have $u^W_{\lambda} \in W$, $u^K_{\lambda} \in K$ and $\lm(u^K_{\lambda}) = u_{\lambda}$.
\end{remark}
...
By \Cref{lmm:9}, \Cref{rmk:3} and the fact that $K$ is a vertex Poisson submodule (not just a submodule), we have
\begin{equation*}
K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  Pick $v \in K'$ such that $\lm(v) = u_{\eta}$ and some power product $p_{\tau} \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_{\tau}v)) = u_{\lambda}$ (this can be done because of \Cref{rmk:4} and the fact that $\lm$ is multiplicative).
...
\begin{proof}
  ...
  By \Cref{rmk:5}, $u_{[\eta, 1, \dots, 1]} \mid u_{\lambda}$ where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:5}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $u_{\eta} \nmid u_{\lambda}$.
...
\begin{lemma}
  ...
  We have:
  \begin{align*}
    p^{1/16}_{>2}(t, q) &= f_{2, 2, 0, 0}(t, q), &p^{1/16}_{>2, 1}(t, q) &= f_{4, 2, 1, 1}(t, q) \\
    p^{1/16}_{>2, 1, 1}(t, q) &= f_{9, 4, 5, 4}(t, q) + f_{5, 3, 2, 2}(t, q), &p^{1/16}_{>3, 1, 1, 1}(t, q) &= f_{7, 3, 3, 3}(t, q).
  \end{align*}
\end{lemma}
