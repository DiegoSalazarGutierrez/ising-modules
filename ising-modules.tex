...
\begin{proposition}
  \label{prp:3}
  Let $M$ be a vector space, let $(M^p)_{p \in \mathbb{Z}}$ be a family of subspaces of $M$ satisfying properties \emph{(i)--(iii)} of \Cref{prp:2}, and let $(B^p)_{p \in \mathbb{N}}$ be a increasing family of sets satisfying $B^p \subseteq M^p$ for $p \in \mathbb{N}$.
