...
By \Cref{prp:2}(vii), we can define a diagonalizable operator $H^M \in \End(\gr^G(M))$ by setting $H^M(\alpha^p_M(u)) = \alpha^p_M(L^M_0u)$ for $p \in \mathbb{N}$ and $u \in G^pM$.
...
\begin{proposition}
  \label{prp:3}
  Let $M$ be a vector space, let $(M^p)_{p \in \mathbb{Z}}$ be a family of subspaces of $M$ satisfying properties \emph{(i)--(iii)} of \Cref{prp:2}, and let $(B^p)_{p \in \mathbb{N}}$ be an increasing family of sets satisfying $B^p \subseteq M^p$ for $p \in \mathbb{N}$.
  ...
  In particular, if $\{v + M^{p - 1} \mid p \in \mathbb{N}, v \in B^p \setminus B^{p - 1}\}$ is a basis of $\gr(M) = \bigoplus_{p \in \mathbb{N}}M^p/M^{p - 1}$, then $\bigcup_{p \in \mathbb{N}}B^p$ is a basis of $M$.
\end{proposition}
...
\begin{remark}
  ...
  It is possible to define the standard filtration for admissible modules (see \cite[Definition 3.3]{dong_twisted_1998}), and \Cref{prp:4} still holds.
\end{remark}
...
\begin{proof}
  The result follows immediately from \Cref{lmm:7}.
\end{proof}
...
\begin{proof}
  ...
  By \Cref{lmm:8} and \eqref{eq:6}, we have $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}

\begin{remark}
  \label{rmk:5}
  The proof of \Cref{lmm:9} also works for other values of $c = c_{p, q} = 1 - \frac{6(p - q)^2}{pq}$ and $h = h_{m, n} = \frac{(np - mq)^2 - (p - q)^2}{4pq}$, where $p, q \ge 2$ are relatively prime integers, and $m, n$ are integers satisfying $0 < m < p$ and $0 < n < q$, see \cite{wang_rationality_1993}.
\end{remark}
...
\begin{remark}
  \label{rmk:7}
  By construction and \Cref{rmk:6}, for each pivot $\lambda$ of $A^W_n$, we have $u^W_{\lambda} \in W$, $u^K_{\lambda} \in K$ and $\lm(u^K_{\lambda}) = u_{\lambda}$.
\end{remark}
...
By \Cref{lmm:9}, \Cref{rmk:7} and the fact that $K$ is a vertex Poisson submodule (not just a submodule), we have
\begin{equation*}
  K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  We pick $v \in K'$ such that $\lm(v) = u_{\eta}$ and some power product $p_{\tau} \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_{\tau}v)) = u_{\lambda}$ (this can be done because of \Cref{rmk:8} and the fact that $\lm$ is multiplicative).
...
\begin{proof}
  ...
  By \Cref{rmk:9}, $u_{[\eta, 1, \dots, 1]} \mid u_{\lambda}$, where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:9}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $u_{\eta} \nmid u_{\lambda}$.
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  Therefore, by \Cref{lmm:6}, isomorphism \eqref{eq:7} and the equality $\dim(\gr^G(L)_{1/2 + n}) = \dim(L_{1/2 + n})$ for $n \in \mathbb{N}$, we have
  \begin{equation*}
  ...
  By \Cref{lmm:10}, there would exist $u \in K' \cap F_N \subseteq K \cap F_N$ such that $\lm(u) = u_{\eta}$.
  ...
  This observation and \Cref{prp:3} conclude the proof of \Cref{thr:4}.
\end{proof}
