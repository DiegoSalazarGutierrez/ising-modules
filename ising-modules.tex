...
\begin{proof}
  ...
  By \Cref{lmm:9} and \eqref{eq:3} we have $v_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(v)) = \alpha^p(v_{(-1)}\vachalf) \in K$, finishing the proof.  
\end{proof}
...
By \Cref{lmm:10}, the construction of the elements $m^K_\lambda$ and the fact that $K$ is a Poisson submodule (not just a submodule) we have
\begin{equation*}
K' \subseteq K.
\end{equation*}
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  Therefore by \Cref{lmm:6}, isomorphism~\eqref{eq:5} and the isomorphism between $\gr^G(L)_{1/2 + n}$ and $L_{1/2 + n}$ of \Cref{prp:6} we have
  \begin{equation*}
