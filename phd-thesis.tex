...
\end{enumerate}
Thus, an admissible module is a pair $(M, \bigoplus_{n \in \mathbb{Z}}M(n))$ satisfying the above conditions, though we often omit the grading from the notation.
...
Indeed, one of our objectives is to study $M(1/2, 1/2)$ and $M(1/2, 1/16)$, which are $\mathbb{Q}$-graded.
...
\subsection{The Zhu $C_2$-algebra and its modules}
...
The fact that $\gr_F(V)$ is a vertex Poisson algebra implies that $R_V$ is a Poisson algebra with operation given as follows.
For $a, b \in V$, we set:
\begin{align*}
...
\begin{example}
  \label{exa:18}
  Let $A$ be an associative almost commutative algebra with $A$ with a unit $1$ satisfying (i)--(iv) of ...
  We know that $\gr(A)$ an associative commutative algebra with a unit $\gamma^0(1)$.
...
In this appendix alone, by an algebra we will mean a commutative associative algebra with a unit.
