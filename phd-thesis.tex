...
We can extend the definition of $\lm$ (\emph{leading monomial}) as follows.
...
\begin{proof}
  ...
  By \Cref{lmm:35} and \eqref{eq:48}, we have $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}
...
If we were using only the degree reverse lexicographical order with TOP, then we would get $L_{-3}L_{-1}\vachalf$ as leading monomial instead.
