...
\begin{remark}
  \label{rmk:38}
  We probably cannot deduce the right Leibniz rule from the left Leibniz rule (c.f.\ \Cref{rmk:53})
\end{remark}
...
If $V$ is graded and $M$ is a graded $V$-module, then, as we explained in \Cref{sec:filtr-vert-algebr}, $\gr_F(M)$ is a graded $\gr_F(V)$-module.
...
If $f: R_1 \to R_2$ is a homomorphism of finitely generated algebras, then $Jf: JR_1 \to JR_2$ is defined by requiring that $Jf(\partial_1^n(\inc_1(x))) = \partial^n_2(\inc_2(f(x)))$ for $x \in R_1$ and $n \in \mathbb{N}$ (c.f.\ \Cref{rmk:54}).
