...
\DeclareMathOperator{\Alg}{Alg^f}
\DeclareMathOperator{\DAlg}{DAlg^f}
...
\end{align*}
called $\lambda$-bracket, satisfy the axioms of a Lie conformal superalgebra, which is very similar to a Lie superalgebra, together with quasicommutativity, quasiassociativity and the noncommutative Wick formula.
...
A module $M$ over a vertex superalgebra $V$ can be defined as a vector superspace $M$ together with a linear parity preserving map $Y^M: V \to \End(M)[[z^{\pm 1}]]$ satisfying the vacuum axiom and the Borcherds identity.
...
However, the Virasoro vertex algebras are foundational in the sense that most vertex algebras $V$ are required to be conformal, which means there is a distinguished vector $\omega \in V$ satisfying the Virasoro relations.
...
Given a $\mathbb{Z}$-graded vertex algebra $V$, the Zhu algebra $\Zhu(V)$ is a usual associative algebra with unit that describes the irreducible admissible $V$-modules by doing the following: given an irreducible admissible $V$-module $M$, the space of lowest weight vectors $M(0)$ is a $\Zhu(V)$-module.
...
In this thesis, we focus on the Ising model $\Vir_{3, 4} = \Vir_{1/2}$, which is conformal, $\mathbb{N}$-graded, $C_2$-cofinite and rational.
The irreducible modules over $\Vir_{3,4}$ are $\Vir_{3, 4} = L(1/2, 0)$, $L(1/2, 1/2)$ and $L(1/2, 1/16)$, which are irreducible highest weight representations of the Virasoro Lie algebra.
To do so, we study filtrations of vertex algebras and their modules.
...
This thesis is organized as follows: In \Cref{sec:vert-supera-their}, we introduce the notion of vertex superalgebras and their modules while trying to compile several results.
...
In the appendices, we recall some results about almost commutative algebras, Poisson algebras and Jet algebras, we show the SageMath \cite{sagemath} program used to compute a Poisson submodule related to the exceptional partitions appearing in $R^{1/2}$, and we briefly consider the case $L(1/2, 1/16)$.
...
A homomorphism $f: (A, \partial_1) \to (A_2, \partial_2)$ of differential superalgebras is a a linear parity preserving map such that $f\circ\partial_1 = \partial_2\circ f$.
...
Homomorphisms of graded vertex superalgebras are assumed to respect the gradings, i.e., if $f: V_1 \to V_2$ is a homomorphism of graded vertex superalgebras, then $f\circ H^{V_1} = H^{V_2}\circ f$, where $H^{V_1}$ is the Hamiltonian of $V_1$, and $H^{V_2}$ is the Hamiltonian of $V_2$.
...
\begin{example}[\emph{Sugawara construction} {\cite[Theorem 5.7]{kac_vertex_1998}}]
  \label{exa:9}
  We recall the universal affine vertex superalgebra of level $k$ of \Cref{exa:6} and assume that $\mathfrak{g}$ is simple and finite dimensional.
...
A \emph{conformal vertex superalgebra homomorphism} $(V_1, \omega_1) \to (V_2, \omega_2)$ is a homomorphism of vertex superalgebras such that $\omega_1 \mapsto \omega_2$.

\begin{remark}
  \label{rmk:21}
  Because homomorphisms of conformal vertex superalgebras respect the conformal vectors, they automatically respect the grading as well.
\end{remark}
...
\begin{equation*}
  V^{\Gen}_{p(x)} = \{a \in V \mid \text{there is }k \in \mathbb{N}\text{ such that } p(H)^ka = 0\}.
\end{equation*}
...
\begin{lemma}
  ...
  \item If $U$ is a subspace of $V^{\Gen}$ such that $H(U) \subseteq U$, then $U$ is graded, i.e.,
    \begin{equation*}
...
An ideal $I$ of a graded vertex superalgebra $V$ is called \emph{graded} if $H(I) \subseteq I$ or, equivalently, $I = \sum_{\Delta \in \mathbb{C}}I \cap V_{\Delta}$ (see \Cref{lmm:9}).
...
\begin{theorem}[{\cite{gorelik_simplicity_2007}}]
  ...
  In the expression
  \begin{equation*}
    a_{p, q} = \sum_{\substack{i_1 \ge \dots \ge i_k \ge 2 \\ i_1 + \dots + i_k = (p - 1)(q - 1)}}L_{-i_1}\dots L_{-i_k}\vac,
  \end{equation*}
  the coefficient of $L_{-2}^{(p - 1)(q - 1)/2}$ is nonzero.
\end{theorem}
...
\end{equation*}
We obtain the category $V\text{-Mod}$ of $V$-modules which is abelian.
...
\begin{equation}
  \label{eq:25}
  [H^M, a^M_{(n)}] = -(n + 1)a^M_{(n)} + (Ha)^M_{(n)} \quad \text{for }a \in V\text{ and }n \in \mathbb{Z}
\end{equation}
and to
\begin{equation}
  \label{eq:26}
  [H^M, a^M_{(n)}] = (\Delta_a - n - 1)a^M_{(n)} \quad \text{for }a \in V\text{ homogeneous and }n \in \mathbb{Z}.
\end{equation}
...
\begin{equation}
  \label{eq:27}
  [H^M, a^M_n] = -na^M_n \quad \text{for }a \in V\text{ homogeneous and }n \in \mathbb{Z} - \Delta_a.
\end{equation}
...
Homomorphisms of graded modules over graded vertex superalgebras are assumed to respect the gradings, i.e., if $f: M_1 \to M_2$ is a homomorphism of graded $V$-modules, then $f\circ H^{M_1} = H^{M_2}\circ f$, where $H^{M_1}$ is the Hamiltonian of $M_1$, and $H^{M_2}$ is the Hamiltonian of $M_2$.
...
The $(V, \omega)$-modules form a subcategory of $V\text{-Mod}$.
...
We usually require $M_h \neq 0$, and in this case, the top degree component is well defined.


\begin{remark}
  \label{rmk:26}
  Let $(V, \omega)$ be a conformal vertex algebra, and let $M_1$ and $M_2$ be $(V, \omega)$-modules.
  A homomorphism $f: M_1 \to M_2$ automatically respects the gradings of $M_1$ and $M_2$.
\end{remark}
...
\begin{enumerate}
  \item $M(n) = 0$ for $n < 0$;
  \item $a^M_{(n)}M(m) \subseteq M(m + \Delta_a - n - 1)$ for $a \in V$ homogeneous and $m, n \in \mathbb{Z}$, or, equivalently, $a^M_nM(m) \subseteq M(m - n)$ for $a \in V$ homogeneous, $m \in \mathbb{Z}$ and $n \in \mathbb{Z}$.
\end{enumerate}
...
We note that shifting the grading of an admissible $V$-module gives an isomorphic admissible $V$-module, and that submodules of admissible $V$-modules.
When $M \neq 0$ is an admissible $V$-module, we can always shift the grading so that $M(0) \neq 0$.
Thus, we have a functor
\begin{equation*}
  \Shift: \{\text{admissible $V$-modules}\} \to \{\text{admissible $V$-modules}\},
\end{equation*}
which is obviously an equivalence of categories.
...
\begin{example}
  \label{exa:11}
  Let $V$ be a highest weight representation of $\Vir$ with highest weight $(c, h)$ (see \Cref{sec:high-weight-repr} ahead) and highest weight vector $v$.
...
Consider the endomorphism $L^M_0 \in \End(M)$, which in general is not diagonalizable.
By \Cref{lmm:10}, we have a direct sum decomposition
\begin{equation*}
...
\begin{equation}
  \label{eq:28}
  \begin{split}
    \Lambda^M &= \{\Delta \in \mathbb{C} \mid M^{\Gen}_{\Delta} \neq 0\text{ and for }n \in \mathbb{Z}_+, M^{\Gen}_{\Delta - n} = 0\}, \\
    M^{\Gen}(n) &=
                  \begin{cases}
                    \sum_{\Delta \in \Lambda^M}M^{\Gen}_{\Delta + n} &\text{if }n \ge 0; \\
                    0 &\text{if }n < 0.
                  \end{cases}
  \end{split}
\end{equation}
...
\begin{equation}
  \label{eq:29}
  \text{for }\Delta \in \mathbb{C},\text{there is }N \in \mathbb{N}\text{ such that for }n \in \mathbb{N}\text{ with }n \ge N, M^{\Gen}_{\Delta - n} = 0.
\end{equation}
...
\begin{proposition}
  ...
  \item If $M$ satisfies \eqref{eq:29}, then $M^{\Gen}$ is an admissible $V$-module with grading given by \eqref{eq:28};
  \item If $f: M_1 \to M_2$ is a homomorphism of $V$-modules, then $f(M^{\Gen}_{1\Delta}) \subseteq M^{\Gen}_{2\Delta}$ for $\Delta \in \mathbb{C}$.
    In particular, $f(M^{\Gen}_1) \subseteq M^{\Gen}_2$.
  \end{enumerate}
\end{proposition}

\begin{proof}
  We omit superindices in this proof.
  \begin{enumerate}
  \item From the formula $[L_0, a_n] = -na_n$ given in \eqref{eq:17}, we inductively get the formula
    \begin{equation*}
  ...
  \item From (i), we immediately get that $M^{\Gen}$ is a submodule of $M$.
  \item If this condition is satisfied, then we have $M^{\Gen} = \bigoplus_{n \in \mathbb{Z}}M^{\Gen}(n)$.
    From (i), we get that this grading makes $M^{\Gen}$ into an admissible $V$-module.
  \item This follows straight from the definitions. \qedhere
  \end{enumerate}
\end{proof}
We have constructed a functor
\begin{align*}
  \Gen: \{\text{$V$-modules satisfying }\eqref{eq:29}\} \to \{\text{admissible $V$-modules}\}.
\end{align*}
...
\begin{remark}
  \label{rmk:27}
  We followed \cite{dong_twisted_1998} when we defined admissible modules and their homomorphism.
...
\begin{enumerate}
\item $\dim(M_{\Delta}) < \infty$ for all $\Delta \in \mathbb{C}$;
\item For $\Delta \in \mathbb{C}$, there is $N \in \mathbb{N}$ such that for $n \in \mathbb{N}$ with $n \ge N$, $M_{\Delta - n} = 0$.
\end{enumerate}
The ordinary $V$-modules form a subcategory of $V\text{-Mod}$ and they satisfy \eqref{eq:29}.
...
A positive energy representation $M$ of the conformal vertex algebra $(V, \omega)$ is ordinary if for $n \in \mathbb{N}$, $\dim(M_{h + n}) < \infty$, where $M_h$ is a degree component of $M$.
...
\begin{example}
  ...
  In fact, as we will see in \Cref{thr:44}, a complete list of all the irreducible modules of $\Vir_c$ is given in that article.
...
By \eqref{eq:30} and \Cref{thr:20}(vi), the bracket in $[V]_0$ is given by
\begin{equation}
  \label{eq:32}
...
\begin{proof}
  If $a \in V$ is homogeneous, then by \Cref{thr:20}(v) and \eqref{eq:31},
  \begin{equation*}
...
By \eqref{eq:35}, the character of a Verma representation is given by
\begin{equation*}
...
By \Cref{lmm:18}(iii), $M(c, h)$ has a unique maximal subrepresentation $J(c, h)$.
The quotient
\begin{equation*}
  L(c, h) = M(c, h)/J(c, h)
\end{equation*}
...
\begin{remark}
  \label{rmk:29}
  By \eqref{eq:34}, a highest weight vector is singular.
  By \eqref{eq:33}, we can write a singular vector $u$ as $u = \sum_{j \in \mathbb{N}}u_j$, where $u_j \in V_{h + j}$ for $j \in \mathbb{N}$.
...
Contravariance of $\langle\bullet| \bullet\rangle$ and \eqref{eq:36} impose conditions on the highest weight $(c, h)$.
Let $a \in U(\Vir)$.
Then
\begin{equation*}
...
We note, however, that initially we cannot define the form using \eqref{eq:36} for a general highest weight representation because there may be linear dependences between terms of the form $L_{\lambda}v$ for a partition $\lambda$, so we may ask if it is well defined in the first place.
...
\begin{theorem}[{\cite[Proposition 3.4]{kac_bombay_2013}}]
  ...
  We pick a highest weight vector $v$, and we define the form $\langle\bullet| \bullet\rangle$ on $M(c, h)$ using equation \eqref{eq:36}.
  Then:
  \begin{enumerate}
  ...
  \item Every highest weight representation $V$ of $\Vir$ with highest weight $(c, h)$ and highest weight vector $v'$ carries a contravariant form such that $\langle v'| v'\rangle = 1$.
    This form on $V$ is defined by \eqref{eq:36} with $v'$ in place of $v$ and satisfies properties \emph{(i)--(iii)} of this Theorem.
...
\begin{remark}
  \label{rmk:32}
  For $c \in \mathbb{C}$, $\Vir^c = M(c, 0)/U(\Vir)\{L_{-1}\vac\}$ and $\Vir_c = L(c, 0)$ because by \Cref{rmk:24}, the ideals of $\Vir^c$ are in bijection with the submodules of $M(c, 0)/U(\Vir)\{L_{-1}\vac\}$.
  If $c = c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, then the singular vector of $\Vir^c$ can be computed by solving the equation $L_1u = L_2u = 0$ for $u \in \Vir^c_{(p - 1)(q - 1)}$ (see \Cref{thr:23} and \Cref{lmm:19}).
\end{remark}
...
This naturally leads to a functor $\Omega: \text{$V$-Mod} \to \Zhu(V)\text{-Mod}$.
...
Recall from \Cref{sec:lie-algebr-assoc} that $[V]$ is a $\mathbb{Z}$-graded Lie algebra with a triangular decomposition $[V] = [V]_+ \oplus [V]_0 \oplus [V]_-$.
Let $W$ be a $[V]$-module.
We denote by $\Omega(W)$ the subspace of $W$ of \emph{lowest weight vectors} with respect to the triangular decomposition of $[V]$, that is,
\begin{equation*}
  \Omega(W) = \{u \in W \mid [V]_-u = 0\}.
\end{equation*}
If $M$ is a $V$-module, then by \Cref{lmm:15}, $[M]$ is a $[V]$-module, and we define the space of \emph{lowest weight vectors of $M$} as
\begin{equation*}
  \Omega(M) = \Omega([M]).
\end{equation*}
...
\begin{equation*}
  \Omega: \text{$V$-Mod} \to \Zhu(V)\text{-Mod}.
\end{equation*}
...
Thanks to \Cref{thr:36}, we can consider $U$ a module over the Lie algebra $[V]_0$, and then $U$ is a module over the Lie algebra $[V]_- \oplus [V]_0$ by letting $[V]_-$ act trivially.
Thus, we define
\begin{equation*}
...
Furthermore, we recall from \Cref{sec:lie-algebr-assoc} that $H$ is a derivation of $[V]$, and we consider the operator
\begin{equation*}
...
\end{equation*}
However, we are not done yet because all the conditions in \Cref{prp:9} are met except \eqref{eq:23}.
...
\begin{remark}
  ...
  To elaborate a bit more, let $f: V_1 \to V_2$ be a homomorphism of graded vertex algebras.
  We naturally define the functors $\circ f: V_2\text{-Mod} \to V_1\text{-Mod}$ and $\circ \Zhu(f): \Zhu(V_2)\text{-Mod} \to \Zhu(V_1)\text{-Mod}$.
  Then the following diagram of functors commute
  \begin{equation*}
    \begin{tikzcd}
      \Zhu(V_2)\text{-Mod} \arrow[r, "L_{V_2}"] \arrow[d, "\circ \Zhu(f)"] & \{\text{completely reducible admissible $V_2$-modules}\} \arrow[d, "\circ f"] \\
      \Zhu(V_1)\text{-Mod} \arrow[r, "L_{V_1}"]                           & \{\text{completely reducible admissible $V_1$-modules}\}
    \end{tikzcd}
  \end{equation*}
...
\begin{proof}
  ...
  By \Cref{thr:40} and \Cref{thr:41}, the irreducible admissible modules of $\Vir^c$ are $L(\mathbb{C}_h)$ for $h \in \mathbb{C}$.
  By \Cref{thr:27}, for $h \in \mathbb{C}$, $L(\mathbb{C}_h)$ is a smooth $\Vir$-module with central charge $c$.
  Also, $L_0 = (L_{-2}\vac)^{L(\mathbb{C}_h)}_{(1)}$ acts by multiplication by $h$ on $\mathbb{C}_h$ and by \Cref{rmk:34}, $L(\mathbb{C}_h)$ is generated by $\mathbb{C}_h$ as a vertex algebra.
...
\begin{proof}
  ...
  We conclude from \Cref{thr:42} that the $\Vir_c$-module corresponding to $\mathbb{C}_{h_{m, n}}$ is $L(c, h_{m, n})$, and the commutativity of the diagram follows from \Cref{rmk:35}.
\end{proof}

\begin{remark}
  \label{rmk:38}
  It follows from \Cref{thr:44} that the irreducible admissible modules of the Ising model $\Vir_{3, 4}$ are $\Vir_{3, 4} = L(1/2, 0)$, $L(1/2, 1/2)$ and $L(1/2, 1/16)$.
\end{remark}

\begin{theorem}
  \label{thr:45}
  We set $c = c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, and we consider the real number $h_{m, n}$ for integers $m, n$ such that $0 < m < p$ and $0 < n < q$, as defined in \eqref{eq:38}.
...
\begin{proof}
  ...
  \end{equation}
  We are in Subsubcase III$_-$ of \Cref{thr:34}, and we see that the first two elements of the solutions in \eqref{eq:39} are $(n, m)$ and $(n - q, m - p)$ (not necessarily in that order).
\end{proof}
...
Given a vertex Lie superalgebra $V$, we obtain the abelian category $V\text{-Mod}$ of modules over $V$.
...
Given a vertex Poisson algebra $V$, we obtain the abelian category $V\text{-Mod}$ of modules over $V$.
...
\begin{proposition}[{\cite[Proposition 2.3.1]{arakawa_remark_2012}}]
  ...
  \begin{equation*}
    a_{(n)}b = \delta_{n, 0}\{a, b\} \quad \text{for }a, b \in R\text{ and }n \in \mathbb{N}.
  \end{equation*}
\end{proposition}

The vertex Poisson algebra structure on $JR$ given in \Cref{prp:11} for a finitely generated Poisson algebra $R$ will be called the \emph{level 0 vertex Poisson algebra structure}.

If $R $ is a finitely generated Poisson algebra and $M$ is an $R$-module over $R$, then we can verify that $JR \otimes_{R} M$ is $JR$-module by defining the Poisson structure as
\begin{equation*}
  a_{(n)}(b\otimes u) = (a_{(n)}b)\otimes u + \delta_{n, 0}b\otimes\{a, u\} \quad \text{for }a, b \in R\text{ and }u \in M.
\end{equation*}
We have a natural inclusion $\inc: M \hookrightarrow JR \otimes M$, which is a $R$-module homomorphism.
The $JR$-module $M$ together with the $R$-module homomorphism $\inc: M \hookrightarrow JR \otimes M$ satisfy a universal property similar to that of $JR$ (see \Cref{sec:poiss-algebr-their-1}).
...
We have constructed a functor
\begin{equation*}
  JR \otimes \bullet: R\text{-Mod} \to JR\text{-Mod},
\end{equation*}
which is left adjoint to the forgentful functor $JR\text{-Mod} \to R\text{-Mod}$, i.e., for $M \in R\text{-Mod}$ and $N \in JR\text{-Mod}$, we have a natural isomorphism
\begin{equation*}
  \Hom_{JR\text{-Mod}}(JR \otimes M, N) \cong \Hom_{R\text{-Mod}}(M, N).
\end{equation*}
...
\end{equation}
By \eqref{eq:40}, when $\dim(V_{\Delta}) < \infty$ for $\Delta \in \mathbb{C}$, it is natural to define the \emph{refined character of $V$ with respect to the Li filtration} as
\begin{equation*}
...
\begin{align*}
  ...
  \gr_F(f)(\sigma^{V_1}_p(a)) &= \sigma^{V_2}_p(f(a)) \quad \text{for }p \in \mathbb{N}\text{ and }a \in F_pV_1,
\end{align*}
...
\end{equation}
By \eqref{eq:41}, when $\dim(V_n) < \infty$ for $n \in \mathbb{N}$, it is natural to define the \emph{refined character of $V$ with respect to the standard filtration} as
\begin{equation*}
...
\begin{align*}
  ...
  \gr^G(f)(\alpha_{V_1}^p(a)) &= \alpha_{V_2}^p(f(a)) \quad \text{for }p \in \mathbb{N}\text{ and }a \in G^pV_1,
\end{align*}
defines a homomorphism of vertex Poisson algebras.
Therefore, we obtain a functor
\begin{equation*}
...
\begin{example}[$\gr^G(\Vir^c)$]
  ...
  If $s = 0$ or $s = 1$, \eqref{eq:42} is clear.
  We assume $s \ge 2$.
  By \Cref{crl:1},
  \begin{align*}
  ...
  \item $k = 0$ and $n_1 = \dots = n_{s - 1} = 0$.
    ...
    \end{equation*}
  \end{enumerate}
  From these three cases, we obtain \eqref{eq:42}.

  We conclude that we have an algebra isomorphism $\gr^G(\Vir^c) \xrightarrow{\sim} \gr(U(\Vir_{\le -2}))$.
  Composing this with the inverse of the isomorphism $S(\Vir_{\le -2}) \to \gr(U(\Vir_{\le -2}))$ in \Cref{sec:almost-comm-algebr} and taking $(L_n)_{n \le -2}$ as the basis of $\Vir_{\le -2}$, we obtain the following isomorphism of commutative associative algebras with unit
  \begin{align*}
...
\end{equation}
By \eqref{eq:43}, when $\dim(M)_{\Delta} < \infty$ for $\Delta \in \mathbb{C}$, it is natural to define the refined series of $M$ with respect to the Li filtration as
\begin{equation*}
...
\begin{align*}
  ...
  \gr_F(f)(\sigma^{M_1}_p(u)) &= \sigma^{M_2}_p(f(u)) \quad \text{for }p \in \mathbb{N}\text{ and }u \in F_pM_1,
\end{align*}
...
\end{equation}
By \eqref{eq:44}, when $M$ is ordinary, which means $\dim(M_{h + n}) < \infty$ for $n \in \mathbb{N}$, it is natural to define the refined series of $M$ with respect to the standard filtration as
\begin{equation*}
...
\begin{align*}
  ...
  \gr^G(f)(\alpha^p_{M_1}(u)) &= \alpha^p_{M_2}(f(u)) \quad \text{for }p \in \mathbb{N}\text{ and }u \in G^pM_1,
\end{align*}
...
\begin{remark}
  \label{rmk:45}
  We could have done the theory of standard filtrations of modules for admissible modules over $\mathbb{N}$-graded vertex algebras (or maybe even over admissible $\mathbb{Z}$-graded vertex algebras, as it is done in \cite{li_vertex_2004}).
...
\begin{example}
  \label{exa:16}
  We pick $c \in \mathbb{C}$.
  We now compute $R_{\Vir^c}$ and $R_{\Vir_c}$.
  ...
  \end{align*}
  Applying the functor $R$, we obtain a surjective homomorphism
  \begin{equation*}
...
The \emph{associated variety} $X_V$ and the \emph{singular support} $SS(V)$ of a vertex algebra $V$ (no condition on $V$) are defined by:
\begin{align*}
  X_V &= \Spec(R_V), \\
  SS(V) &= \Spec(\gr_F(V)).
\end{align*}
...
\begin{proof}
  ...
  By \eqref{eq:45}, we can write $b = \sum_{n \in \mathbb{N}}b_n$, where $b_n \in W \cap V_n$ for $n \in \mathbb{N}$.
...
\begin{remark}
  \label{rmk:46}
  For a more generalization of \Cref{thr:52} assuming $V$ is a lower truncated $\mathbb{Z}$-graded vertex algebra, see \cite[Theorem 4.8]{li_abelianizing_2005}.
\end{remark}
...
\begin{remark}
  ...
  From the definition of the Zhu $C_2$-algebra, we see that $R_V$ is isomorphic to $\mathbb{C}[t]/(t^2)$ as an algebra, and the later is generated by the single element $t + (t^2)$.
...
\begin{proof}\leavevmode
  ...
  \item[(ii)$\implies$(iv)] If $\dim(X_V) = 0$, then by \Cref{thr:59}, $\dim(JX_V) = 0$.
...
\begin{theorem}[{\cite[Theorem 5.8]{abe_rationality_2003}}]
  \label{thr:54}
...
\begin{example}
  ...
  By \Cref{exa:16}, \Cref{exa:12} and \Cref{thr:54}, $\Vir_c$ is regular.
\end{example}

Let $V$ be a vertex algebra, and let $M$ be a module over $V$.
By \Cref{lmm:24}, we have
\begin{equation*}
  C_2M = \vspan\{a_{(-2)}b \mid a, b \in V\} = F_1M.
\end{equation*}
We define the Zhu $C_2$-module of $M$ by
\begin{equation*}
...
\begin{align*}
  \sigma_0(a)\cdot\sigma^M_0(u) &= \sigma^M_0(a^M_{(-1)}u), \\
  \{\sigma_0(a), \sigma^M_0(u)\} &= \sigma^M_0(a^M_{(0)}u).
\end{align*}
We have constructed a functor
\begin{equation*}
  R: V\text{-Mod} \to R_V\text{-Mod}.
\end{equation*}
...
The \emph{associated variety} $X_M$ and the \emph{singular support} $SS(M)$ of a $V$-module $M$ are defined by:
\begin{align*}
  X_M &= \Supp_{R_M}(R_V), \\
...
As we have explained in \Cref{sec:vert-poiss-algeb-2}, we can consider $JR_V\otimes R_M$ as a $JR_V$-module.
...
By the universal property of the $JR_V$-module $JR_V \otimes R_M$ (see \Cref{sec:vert-poiss-algeb-2}), there is a $JR_V$-module homomorphism $\phi_M: JR_V \otimes_{R_V} R_M \to \gr_F(M)$ such that the following diagram commutes
\begin{equation*}
  \begin{tikzcd}
...
\begin{proof}
  ...
  We define
  \begin{equation*}
    M^0 = \vspan\{a^M_{(-1)}u \mid a \in V\text{ homogeneous and }u \in T\}.
  \end{equation*}
...
\begin{theorem}
  ...
  Then:
  \begin{enumerate}
  \item Any $V$-module $M$ satisfies $M = M^{\Gen}$ and \eqref{eq:29};
  \item We have $\For\circ\Gen = \Id_{{\emph{\text{$V$-Mod}}}}$ and $\Gen\circ\For = \Shift$.
    In particular, both $\For$ and $\Gen$ are equivalences of categories;
  \item If $M$ is admissible $V$-module, then $M$ is irreducible as an admissible $V$-module if and only if $M$ is irreducible as a $V$-module;
...
\begin{proof}\leavevmode
  \begin{enumerate}
  \item The equality $M = M^{\Gen}$ is proved in \cite[Proposition 5.7]{abe_rationality_2003} and \eqref{eq:29} is proved in \cite[Lemma 5.6]{abe_rationality_2003}.
  \item The equality $\For\circ\Gen = \Id_{\text{$V$-Mod}}$ is obvious.
    The equality $\Gen\circ\For = \Shift$ is proved in \cite[Lemma 5.6]{abe_rationality_2003}.
  \item We assume that $M$ is an irreducible admissible $V$-module.
    By (ii), we can assume that $M$ has the grading $\bigoplus_{n \in \mathbb{N}}M^{\Gen}_{n \in \mathbb{Z}}$
    Let $N$ be a submodule de $M$.
    Then $L_0^M(N) \subseteq N$.
    By \Cref{lmm:10}(iii), $N = \sum_{\Delta \in \mathbb{C}}N \cap M^{\Gen}_{\Delta}$.
    This implies that $N = \sum_{n \in \mathbb{N}}N \cap M^{\Gen}(n)$, i.e., $N$ is a graded submodule of $M$.
    Therefore, $N = 0$.
    In conclusion, $M$ is irreducible as a $V$-module.
    If $M$ is an admissible $V$-module that is irreducible as a $V$-module, then it clearly is irreducible as an admissible $V$-module.
...
By \Cref{exa:16} and \Cref{exa:17}, the Ising model $\Vir_{3, 4} = \Vir_{1/2}$ satisfies these conditions. 

\section{PBW basis of $L(1/2, 1/2)$ and its refined character}
\label{sec:pbw-basis-l12}

In this section, our objective is to determine a monomial basis of $L(1/2, 1/2)$ explicitly and a formula for the refined character of $\gr^G(L(1/2, 1/2))$ with respect to the standard filtration, as introduced in \Cref{sec:filtr-modul-over}.
...
As we can see, the arguments are similar to those in \cite{andrews_singular_2022}, but there are some differences such as the use of the Zhu algebra here but not in \cite{andrews_singular_2022} and the fact that we do not have a differential available (see \Cref{rmk:42}).
...
\end{equation*}
We set $I = \ker(\gr^G(\pi_0))$, following the notation of \cite{andrews_singular_2022}, and we recall the definition of $K$ given in \eqref{eq:47}.
...
\begin{proof}
  ...
  \end{equation*}
  where $W$ is defined in \eqref{eq:46}.
  ...
  By \Cref{lmm:35} and \eqref{eq:48}, $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  Therefore, by \Cref{lmm:33}, isomorphism \eqref{eq:49} and the isomorphism between $\gr^G(L)_{1/2 + n}$ and $L_{1/2 + n}$ of \Cref{prp:20}, we have
  \begin{equation*}
  ...
  On the other hand, we have $B_N \subseteq \overline{B_N}$ because if $u_{\lambda} \in B_N$ and $\eta \in \overline{R}$, then we have $u_{\eta} \nmid u_{\lambda}$, because if we had $u_{\eta} \mid u_{\lambda}$, then by \Cref{lmm:37}, there would exist $u \in K' \cap F_N \subseteq K \cap F_N$ such that $\lm(u) = u_{\eta}$, then by the definition of Gr\"{o}bner basis, there would exist $v \in G_N$ such that $\lm(v) \mid \lm(u)$, implying $\lm(v) \mid u_{\lambda}$, a contradiction.
  As both $B_N$ and $\overline{B_N}$ are finite sets and $|B_N| = |\overline{B_N}|$, we get $B_N = \overline{B_N}$.
  We see that the family of bases given by \eqref{eq:50} satisfies the property that one basis is contained in the next one, and this concludes the proof of \Cref{thr:4}.
\end{proof}
...
\begin{proof}
  This follows from the left Leibniz rule and anticommutativity of the Poisson bracket.
\end{proof}

\begin{example}
  \label{exa:18}
  Let $A$ be an associative almost commutative algebra with $A$ with unit $1$ satisfying (i)--(iv) of \Cref{sec:almost-comm-algebr}.
  We know that $\gr(A)$ an associative commutative algebra with unit $\gamma^0(1)$.
...
A Poisson $A$-module or a Poisson module over $A$ is a $A$-module $M$ in the usual associative sense equipped with a bilinear map $\{\bullet, \bullet\}: A \times M \to M$ which makes $M$ a Lie algebra module over $A$ such that for $a, b \in A$ and $u \in M$:
\begin{enumerate}
...
The categories $\Alg$ of finitely generated algebras and $\DAlg$ of finitely generated differential algebras will appear frequently in this section.
...
We assume $R = \mathbb{C}[x_1, \dots, x_r]/(f_1, \dots, f_s)$ for some polynomials $f_i \in \mathbb{C}[x_1, \dots, x_r]$.
...
\end{equation*}
We consider $\partial$ as a derivation of $JR$, and we have an obvious algebra monomorphism $\inc: R \hookrightarrow JR$.
The fact that $\inc: R \hookrightarrow JR$ satisfies our desired universal property is explained in \cite{arakawa_remark_2012} and \cite{ein_jet_2008}.

We have constructed a functor
\begin{equation*}
  J: \Alg \to \DAlg.
\end{equation*}
which is left adjoint to the forgetful functor $\DAlg \to \Alg$, i.e., for $R \in \Alg$ and $A \in \DAlg$, we have a natural isomorphism
\begin{equation*}
