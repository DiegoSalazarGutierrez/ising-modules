...
\begin{theorem}[{\cite[Theorem 5.3]{dong_twisted_1998}}]
  ...
  \begin{align*}
    \Zhu(V) &\to \End(\Omega(M)), \\
    a + O(V) &\mapsto a^M_{(\Delta_a - 1)} \quad \text{for $a \in V$ homogeneous}
  \end{align*}
...
\begin{theorem}
  ...
  \item We have $\For\circ\Gen = \Id_{\text{\emph{$V$-Mod}}}$ and $\Gen\circ\For = \Shift$.
    In particular, both $\For$ and $\Gen$ are equivalences of categories;
  \item If $M$ is an admissible $V$-module, then $M$ is irreducible as an admissible $V$-module if and only if $M$ is irreducible as a $V$-module;
