...
\begin{example}
  \label{exa:11}
  Let $V$ be a highest weight representation of $\Vir$ with highest weight $(c, h)$ (see \Cref{sec:high-weight-repr}) and highest weight vector $v$.
...
\begin{proof}
  ...
  The assertion follows by applying \Cref{lmm:12} to the vertex algebra $V \otimes R$.
\end{proof}
...
By \eqref{eq:24} and \Cref{thr:20}(vi), the bracket in $[V]_0$ is given by
\begin{equation}
  \label{eq:26}
...
\begin{proof}
  If $a \in V$ is homogeneous, then by \Cref{thr:20}(v) and \eqref{eq:25},
  \begin{equation*}
...
By \Cref{lmm:15}, we are led to define
\begin{equation*}
  \Lie_0(V) = V/(T + H)V,
\end{equation*}
...
By \Cref{lmm:17}(iii), $M(c, h)$ has a unique maximal subrepresentation $J(c, h)$.
The quotient
\begin{equation*}
  L(c, h) = M(c, h)/J(c, h)
\end{equation*}
...
\begin{proof}
  ...
  If $\langle\bullet| \bullet\rangle$ is unitary, then by \Cref{lmm:16}, $U \cap V_{h + j} \oplus (U \cap V_{h + j})^{\perp|V_{h + j}} = V_{h + j}$ for $j \in \mathbb{N}$ because all the vector spaces $V_{h + j}$ are finite dimensional.
...
\begin{proof}
  ...
  By \Cref{lmm:17}(ii), either $U = V$ or $U = 0$.
...
\begin{remark}
  ...
  We merely need to find the levels at which the singular vectors that generate $J(c, h)$ lie and then by \Cref{lmm:18}, we have to solve the linear equations $L_1u = L_2 u = 0$ assuming $u$ lies in the right level to obtain our desired singular vector.
\end{remark}

\begin{remark}
  \label{rmk:29}
  For $c \in \mathbb{C}$, $\Vir^c = M(c, 0)/U(\Vir)\{L_{-1}\vac\}$ and $\Vir_c = L(c, 0)$ because by \Cref{rmk:22}, the ideals of $\Vir^c$ are in bijection with the submodules of $M(c, 0)/U(\Vir)\{L_{-1}\vac\}$.
  If $c = c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, then the singular vector of $\Vir^c$ can be computed by solving the equation $L_1u = L_2u = 0$ for $u \in \Vir^c_{(p - 1)(q - 1)}$ (see \Cref{thr:23} and \Cref{lmm:18}).
\end{remark}
...
\begin{proof}
  We know $\Lie_0(V)$ is isomorphic to $[V]_0$ (see \Cref{sec:lie-algebr-assoc}) and by \Cref{lmm:19}(iii), there is a natural linear epimorphism
  \begin{align*}
  ...
  By \Cref{lmm:20}(iii) and the definition of the bracket of $\Lie_0(V)$, we have
  \begin{align*}
...
\end{equation*}
If $M$ is a $V$-module, then by \Cref{lmm:14}, $[M]$ is a $[V]$-module, and we define
\begin{equation*}
  \Omega(M) = \Omega([M]).
\end{equation*}
...
Thanks to \Cref{thr:35}, we can consider $U$ a module over the Lie algebra $[V]_0$, and then $U$ is a module over the Lie algebra $[V]_- \oplus [V]_0$ by letting $[V]_-$ act trivially.
Thus, we define
\begin{equation*}
...
Taking a hint from \Cref{lmm:14}, we define
\begin{equation*}
...
\begin{remark}
  ...
  \item This is not even a functor.
    ...
    On the other hand, if we use the definition of $\Omega$ given in \cite{dong_twisted_1998}, the problem disappears (see \Cref{exa:12}).
...
\begin{proof}
  ...
  By \Cref{thr:39} and \Cref{thr:40}, the irreducible admissible modules of $\Vir^c$ are $L(\mathbb{C}_h)$ for $h \in \mathbb{C}$.
  By \Cref{thr:27}, for $h \in \mathbb{C}$, $L(\mathbb{C}_h)$ is a smooth $\Vir$-module with central charge $c$.
  Also, $L_0 = (L_{-2}\vac)^{L(\mathbb{C}_h)}_{(1)}$ acts by multiplication by $h$ on $\mathbb{C}_h$ and by \Cref{rmk:31}, $L(\mathbb{C}_h)$ is generated by $\mathbb{C}_h$ as a vertex algebra.
...
\begin{proof}
  ...
  We conclude from \Cref{thr:41} that the $\Vir_c$-module corresponding to $\mathbb{C}_{h_{m, n}}$ is $L(c, h_{m, n})$, and the commutativity of the diagram follows from \Cref{rmk:32}.
\end{proof}

\begin{remark}
  \label{rmk:35}
  It follows from \Cref{thr:43} that the irreducible admissible modules of the Ising model $\Vir_{3, 4}$ are $\Vir_{3, 4} = L(1/2, 0)$, $L(1/2, 1/2)$ and $L(1/2, 1/16)$.
\end{remark}
...
\begin{proof}
  ...
  \end{equation}
  We are in Subsubcase III$_-$ of \Cref{thr:33}, and we see that the first two elements of the solutions in \eqref{eq:33} are $(n, m)$ and $(n - q, m - p)$ (not necessarily in that order).
\end{proof}
...
By \Cref{lmm:22}, the Li filtration depends only on $V$ and not on the choice of the strong generators.
...
By \Cref{prp:12} ahead, the standard filtration does not depend on the choice of the strong generators of $V$.

By \Cref{prp:11}(vii), we can define an operator $H \in \End(\gr^G(V))$ as $H(\alpha^p(a)) = \alpha^p(Ha)$ for $p \in \mathbb{Z}$ and $a \in G^pV$.
...
\begin{proof}
  The proof of \Cref{thr:46} also works here.
\end{proof}
...
\begin{proof}
  See the proof of \Cref{prp:16} ahead.
\end{proof}
...
\begin{proof}
  ...
  By \Cref{prp:12}, there exists $N \in \mathbb{N}$ such that $F_pV_{n_0} = 0$ for $p > N$.
...
\begin{remark}
  ...
  \item $R_V$ is finitely generated, but $\gr_F(V)$ is not finitely strongly generated.
  \end{enumerate}
  ...
  We see from the definition of the Zhu $C_2$-algebra that $R_V$ is isomorphic to $\mathbb{C}[t]/(t^2)$ as an algebra, and the later is generated by the single element $t + (t^2)$.
  However, $V$ cannot be finitely strongly generated because it has uncountable dimension, and we get (iii).

  From this example, we see that the assumption of $V$ being $\mathbb{N}$-graded cannot be removed in \Cref{thr:49}.
  As a comment, there may be a missing hypothesis in \cite[Lemma 3.1.5]{arakawa_remark_2012}.
  The previous example is not exactly a counterexample because it is assumed that $V$ is graded in that article, though that is questionable because the proof of \cite[Lemma 3.1.5]{arakawa_remark_2012} does not make use of this by the looks of it.
  The missing hypothesis is probably $M$ admissible because with that hypothesis, we can define the filtration $\gr^G(M)$ (see the next section) and proceed as in the proof of \Cref{thr:49}.
\end{remark}
...
By \Cref{prp:11}(vii), we can define an operator $H^M \in \End(\gr^G(M))$ as $H^M(\alpha_M^p(u)) = \alpha_M^p(L^M_0u)$ for $p \in \mathbb{Z}$ and $u \in G^pM$.
...
\begin{proof}
  ...
  Therefore, by \Cref{prp:14}(v), $a^{i_1M}_{(-n_1 - 1)}u \in G^{\Delta_{a^{i_1}} + \Delta_u - h - p + n_1}M = G^{n - p}M$.
  ...
  We verify that we obtain an isomorphism $\gr_F(M) \xrightarrow{\sim} \gr^G(M)$ directly from the definitions (cf.\ \Cref{prp:12}).
\end{proof}

\Cref{prp:15} translates into an identity of the refined characters of $\gr_F(M)$ and $\gr^G(M)$, as the following proposition shows.
...
\begin{proof}\leavevmode
  ...
  \item This follows from \Cref{prp:15} and by replacing $p$ by $n - p$ in the following computation
    \begin{align*}
...
\begin{example}[$\gr^G(M(c, h))$]
  ...
  From \Cref{exa:13}, $\gr^G(\Vir^c)$ is isomorphic to $\mathbb{C}[L_{-2}, L_{-3}, \dots]$.
...
As we can see, the arguments are similar to those in \cite{andrews_singular_2022}, but there are some differences such as the use of the Zhu algebra here but not in \cite{andrews_singular_2022} and the fact that we do not have a differential available (see \Cref{rmk:39}).
...
The isomorphism in \Cref{exa:14} justifies the following definitions.
...
\begin{proof}
  ...
  \end{align*}
  The solution to these equations is unique if it exists, and we can verify using \Cref{lmm:25} that
  \begin{align*}
  ...
  We derive the formula $p_{>2}(t, q) = f_{3, 2, 0, 0}(t, q)$ again from \Cref{lmm:25}.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m],
  \end{align*}
  from which, together with the proof of \Cref{lmm:26}, we get
  \begin{align*}
  ...
  \end{align*}
  The formula for $p_{>2, 1}(t, q)$ then follows from \Cref{lmm:25}.
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m, 1],
  \end{align*}
  from which, together with the proof of \Cref{lmm:27}, we get
  \begin{align*}
    p_{>4, 1, 1}(t, q) &= tqp_{>4, 1}(t, q) = f_{6, 4, 2, 2}(t, q), \\
    p_{4, 1, 1}(t, q) &= tqp_{>5, 4, 1}(t, q) = f_{9, 5, 6, 4}(t, q).
  \end{align*}
  The formula for $p_{>3, 1, 1}(t, q)$ then follows from \Cref{lmm:25}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  together with \Cref{lmm:26}, \Cref{lmm:27} and \Cref{lmm:28}.
\end{proof}
...
\begin{proof}
  This follows from \Cref{lmm:29} by setting $t = 1$ and \cite[Theorem 4]{andrews_singular_2022} together with \Cref{lmm:25}.
\end{proof}
...
\begin{proof}
  The result follows immediately from \Cref{lmm:31}.
\end{proof}
...
\begin{proof}
  By \Cref{thr:43}, the following diagram commutes
  \begin{equation*}
    \begin{tikzcd}
  ...
  By \Cref{lmm:32} and \eqref{eq:42}, $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}
...
\begin{remark}
  \label{rmk:44}
  By construction and \Cref{rmk:43}, for each pivot $\lambda$ of $A^W_n$, we have $u^W_{\lambda} \in W$, $u^K_{\lambda} \in K$ and $\lm(u^K_{\lambda}) = u_{\lambda}$.
\end{remark}
...
By \Cref{lmm:33}, \Cref{rmk:44} and the fact that $K$ is a vertex Poisson submodule (not just a submodule), we have
\begin{equation*}
K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  We pick $v \in K'$ such that $\lm(v) = u_{\eta}$ and some power product $p_{\tau} \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_{\tau}v)) = u_{\lambda}$ (this can be done because of \Cref{rmk:45} and the fact that $\lm$ is multiplicative).
...
\begin{proof}
  ...
  By \Cref{rmk:46}, $u_{[\eta, 1, \dots, 1]} \mid u_{\lambda}$ where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:46}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $u_{\eta} \nmid u_{\lambda}$.
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  We have to keep in mind the isomorphisms given in \Cref{prp:17}.
  ...
  Therefore, by \Cref{lmm:30}, isomorphism \eqref{eq:43} and the isomorphism between $\gr^G(L)_{1/2 + n}$ and $L_{1/2 + n}$ of \Cref{prp:17}, we have
  \begin{equation*}
  ...
  On the other hand, we have $B_N \subseteq \overline{B_N}$ because if $u_{\lambda} \in B_N$ and $\eta \in \overline{R}$, then we have $u_{\eta} \nmid u_{\lambda}$, because if we had $u_{\eta} \mid u_{\lambda}$, then by \Cref{lmm:34}, there would exist $u \in K' \cap F_N \subseteq K \cap F_N$ such that $\lm(u) = u_{\eta}$, then by the definition of Gr\"{o}bner basis, there would exist $v \in G_N$ such that $\lm(v) \mid \lm(u)$, implying $\lm(v) \mid u_{\lambda}$, a contradiction.
  As both $B_N$ and $\overline{B_N}$ are finite sets and $|B_N| = |\overline{B_N}|$, we get $B_N = \overline{B_N}$.
  We see that the family of bases given by \eqref{eq:44} satisfies the property that one basis is contained in the next one, and this concludes the proof of \Cref{thr:4}.
\end{proof}

\begin{proof}[Proof of \Cref{thr:3}]
  We note that the basis of \Cref{thr:4} is also a basis of $\gr^G(L)$ under the isomorphism between $\gr^G(L)$ and $L$ of \Cref{prp:17}.
  ...
  Therefore, $\ch_{\gr^G(L)}(t, q) = q^{1/2}p(t, q)$ and together with \Cref{lmm:29}, we conclude the proof of \Cref{thr:3}.
\end{proof}
...
\begin{proof}[Sketch of proof]
  ...
  \end{align*}
  is a solution to these equations.
  We derive the formula $p_{>2}(t, q) = f_{2, 2, 0, 0}(t, q)$ again from \Cref{lmm:25}.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m],
  \end{align*}
  from which, together with the proof of \Cref{lmm:36}, we get
  \begin{align*}
    p_{>4, 1}(t, q) &= tqp_{>4}(t, q) = f_{6, 4, 1, 1}(t, q), \\
    p_{>5, 4, 1}(t, q) &= tqp_{>5, 4}(t, q) = f_{9, 5, 5, 3}(t, q), \\
    p_{5, 4, 1}(t, q) &= tqp_{5, 4}(t, q) = f_{13, 6, 10, 5}(t, q), \\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m, 1],
  \end{align*}
  from which, together with the proof of \Cref{lmm:37}, we get
  \begin{align*}
    p_{>4, 1, 1}(t, q) &= tqp_{>4, 1}(t, q) = f_{6, 4, 2, 2}(t, q), \\
  ...
  \end{align*}
  The formula for $p_{>2, 1, 1}(t, q)$ then follows from \Cref{lmm:25}.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  from which, together with the proof of \Cref{lmm:36} and \Cref{lmm:25}, we get
  \begin{align*}
  ...
  \end{align*}
  The formula for $p_{>3, 1, 1, 1}(t, q)$ then follows from \Cref{lmm:25}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  together with \Cref{lmm:36}, \Cref{lmm:37}, \Cref{lmm:38}, \Cref{lmm:39} and \Cref{lmm:25}.
\end{proof}
