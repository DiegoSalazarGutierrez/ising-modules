...
We have the natural vector space isomorphisms
\begin{equation*}
  \alpha^p(G^pV_n) \cong G^pV_n/G^{p - 1}V_n \quad \text{for $p, n \in \mathbb{Z}$}
\end{equation*}
and the refined grading
\begin{equation}
  \label{eq:42}
...
\begin{proof}
  See the proof of \Cref{prp:20} ahead.
\end{proof}
...
By \Cref{prp:19} ahead, the standard filtration does not depend on the choice of the strong generators of $V$.

By \Cref{prp:17}(vii), we can define an operator $H^M \in \End(\gr^G(M))$ as $H^M(\alpha_M^p(u)) = \alpha_M^p(L^M_0u)$ for $p \in \mathbb{Z}$ and $u \in G^pM$.
...
\begin{proposition}
  \label{prp:18}
  Let $M$ be a vector space, let $(M^p)_{p \in \mathbb{Z}}$ be a family of subspaces of $M$ satisfying properties \emph{(i)--(ii)} of \Cref{prp:17}, and let $(B^p)_{p \in \mathbb{N}}$ be a mutually disjoint family of sets satisfying $B^p \subseteq M^p$ for $p \in \mathbb{N}$.
  Then $\{v + M^{p - 1} \mid v \in B^p\}$ spans (resp.\ is a basis of) $M^p/M^{p - 1}$ for $p \in \mathbb{N}$ if and only if $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$ (resp.\ $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B^k$ is linearly independent).
  In particular, if $M = \bigcup_{p \in \mathbb{N}}M^p$, $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B^k$ is linearly independent, then $\{v + M^{p - 1} \mid p \in \mathbb{N}, v \in B^p\}$ is a basis of $\gr(M) = \bigoplus_{p \in \mathbb{N}}M^p/M^{p - 1}$.
\end{proposition}

\begin{proof}
  To begin with, we prove that $\{v + M^{p - 1} \mid v \in B^p\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$ if and only if $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$.
  We now assume that $\{v + M^{p - 1} \mid v \in B^p\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$, and let $v \in M^p$.
  By hypothesis, there is $v_{p, 1}, \dots, v_{p, k_p} \in B^p$ and $c_{p, 1}, \dots, c_{p, k_p} \in \mathbb{C}$ such that $v + M^{p - 1} = c_{p, 1}(v_{p, 1} + M^{p - 1}) + \dots + c_{p, k_p}(v_{p, k_p} + M^{p - 1})$.
  This means that $v - (c_{p, 1}v_{p, 1} + \dots +  c_{p, k_p}v_{p, k_p}) \in M^{p - 1}$.
  Continuing this way, we find $v_{p - 1, 1}, \dots, v_{p - 1, k_{p - 1}} \in B^{p - 1}$, $c_{p - 1, 1}, \dots, c_{p - 1, k_{p - 1}} \in \mathbb{C}$, \dots, $v_{0, 1}, \dots, v_{0, k_0} \in B^0$, $c_{0, 1}, \dots, c_{0, k_0} \in \mathbb{C}$ such that $v - (c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0}) \in M^{-1} = 0$.
  Therefore, $v = c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0}$, and $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$.
  If $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$, then clearly $\{v + M^{p - 1} \mid v \in B^p\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.

  Finally, we prove that $\{v + M^{p - 1} \mid v \in B^p\}$ is a basis of $M^p/M^{p - 1}$ for $p \in \mathbb{N}$ if and only if $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B^k$ is linearly independent.
  We now assume that $\{v + M^{p - 1} \mid v \in B^p\}$ is a basis of $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.
  By the last paragraph, $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  If $c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0} = 0$, where $v_{p, 1}, \dots, v_{p, k_p} \in B^p$, $c_{p, 1}, \dots, c_{p, k_p} \in \mathbb{C}$, \dots, $v_{0, 1}, \dots, v_{0, k_0} \in B^0$, $c_{0, 1}, \dots, c_{0, k_0} \in \mathbb{C}$, then $c_{p, 1}(v_{p, 1} + M^{p - 1}) + \dots + c_{p, k_p}(v_{p, k_p} + M^{p - 1}) = 0$.
  By the linear independence of $\{v + M^{p - 1} \mid v \in B^p\}$, $c_{p, 1} = \dots = c_{p, k_p} = 0$.
  Continuing this way, $c_{p - 1, 1} = \dots = c_{p - 1, k_{p - 1}} = \dots = c_{0, 1} = \dots = c_{0, k_0} = 0$, and $\bigcup_{k \in \mathbb{N}}B^k$ is linearly independent.
  To end this proof, we assume that $\bigcup_{k \le p}B^k$ spans $M^p$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B^k$ is linearly independent.
  By the last paragraph, $\{v + M^{p - 1} \mid v \in B^p\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  Let $v_{p, 1}, \dots, v_{p, k_p} \in B^p$ and $c_{p, 1}, \dots, c_{p, k_p} \in \mathbb{C}$ be such that $c_{p, 1}(v_{p, 1} + M^{p - 1}) + \dots + c_{p, k_p}(v_{p, k_p} + M^{p - 1}) = 0$.
  Then $c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} \in M^{p - 1}$, and $c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} = c_{p - 1, 1}v_{p - 1, 1} + \dots +  c_{p - 1, k_{p - 1}}v_{p - 1, k_{p - 1}} + \dots + c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0}$ for some $v_{p - 1, 1}, \dots, v_{p - 1, k_{p - 1}} \in B^{p - 1}$, $c_{p - 1, 1}, \dots, c_{p - 1, k_{p - 1}} \in \mathbb{C}$, \dots, $v_{0, 1}, \dots, v_{0, k_0} \in B^0$, $c_{0, 1}, \dots, c_{0, k_0} \in \mathbb{C}$ because $\bigcup_{k \le p - 1}B^k$ spans $M^{p - 1}$.
  By the linear independence of $\bigcup_{k \in \mathbb{N}}B^k$, $c_{p, 1} = \dots = c_{p, k_p} = 0$, and $\{v + M^{p - 1} \mid v \in B^p\}$ is linearly independent.
\end{proof}
...
\Cref{prp:19} translates into an identity of the refined characters of $M$, as the following proposition shows.
...
\begin{proof}\leavevmode
  \begin{enumerate}
  \item First, we note that $G^{n - p}M_{h + n} = 0$ for $p, n \in \mathbb{N}$ with $p \ge n$ by \Cref{prp:17}(i).
    By \eqref{eq:44} and \Cref{prp:19},
    \begin{equation*}
      \bigcap_{p \in \mathbb{N}}F_pM = \sum_{n \in \mathbb{N}}\left(\left(\bigcap_{p \in \mathbb{N}}F_p\right) \cap M_{h + n}\right) = \sum_{n \in \mathbb{N}}\left(\bigcap_{p \in \mathbb{N}}F_pM_{h + n}\right) = \sum_{n \in \mathbb{N}}\bigcap_{p \in \mathbb{N}}G^{n - p}M_{h + n} = 0.
    \end{equation*}
  \item First, we prove $\ch_M(q) = \ch_{\gr_F(M)}(q) = \ch_{\gr_F(M)}(1, q)$.
    The latter equality follows from the definition of $\gr_F(M)_{\Delta}$ and $\ch_{\gr_F(M)}(t, q)$.
    ...
    By \Cref{prp:17}(i) and \Cref{prp:19}, $F_pM_{h + n} = 0$ for $n \in \mathbb{N}$ and $p > n$.
    This implies $\sigma^M_p(F_pM_{h + n}) = 0$ for $n \in \mathbb{N}$ and $p > n$.
    By \eqref{eq:45} and \Cref{prp:16}(i), for $n \in \mathbb{N}$,
    \begin{align*}
      \dim(\gr_F(M)_{h + n}) &= \sum_{p \in \mathbb{N}}\dim(\sigma^M_p(F_pM_{h + n})) \\
                             &= \sum_{p \in \mathbb{N}}(\dim(F_pM_{h + n}) - \dim(F_{p + 1}M_{h + n})) \\
                             &= \dim(F_0M_{h + n}) \\
                             &= \dim(M_{h + n}).
    \end{align*}

    It remains prove $\ch_M(q) = \ch_{\gr^G(M)}(q) = \ch_{\gr^G(M)}(1, q)$.
    The latter equality follows from the definition of $\gr^G(M)_{h + n}$ and $\ch_{\gr^G(M)}(t, q)$.
    ...
    By \Cref{prp:17}(ii),(iv), $G^pM_{h + n} = M_{h + n}$ for $p, n \in \mathbb{N}$ with $p \ge n$.
    By \eqref{eq:47}, $\alpha_M^p(G^pM_{h + n}) = 0$ for $p, n \in \mathbb{N}$ with $p \ge n + 1$.
    By \Cref{prp:17}(i), $G^{-1}M_{h + n} = 0$ for $n \in \mathbb{N}$.
    By \eqref{eq:47}, for $n \in \mathbb{N}$,
    \begin{align*}
      \dim(\gr^G(M)_{h + n}) &= \sum_{p \in \mathbb{N}}\dim(\alpha_M^p(G^pM_{h + n})) \\
                             ...
                             &= \dim(M_{h + n}).
    \end{align*}
  \item This follows from \Cref{prp:19} and by replacing $p$ by $n - p$ in the following computation.
    \begin{align*}
...
\begin{proposition}
  \label{prp:21}
  Let $(V, \omega)$ be an $\mathbb{N}$-graded conformal vertex algebra, let $M$ be an $h + \mathbb{N}$-graded $(V, \omega)$-module, and let $(B_p)_{p \in \mathbb{N}}$ be a family of mutually disjoint sets such that $B_p \subseteq F_pM$, and all elements of $B_p$ are homogeneous for $p \in \mathbb{N}$.
  Then $\{v + F_{p + 1}M \mid v \in B_p\}$ spans (resp.\ is a basis of) $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$ if and only if $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$ (resp.\ $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B_k$ is linearly independent).
  In particular, if $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B_k$ is linearly independent, then $\{v + F_{p + 1}M \mid p \in \mathbb{N}, v \in B_p\}$ is a basis of $\gr_F(M)$.
\end{proposition}

\begin{proof}
  To begin with, we prove that $\{v + F_{p + 1}M \mid v \in B_p\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$ if and only if $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$.
  We now assume that $\{v + F_{p + 1}M \mid v \in B_p\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.
  ...
  Continuing this way, we find for $m \in \mathbb{Z}_+$, $v_{p + 1, 1}, \dots, v_{p + 1, k_{p + 1}} \in B_{p + 1}$, $c_{p + 1, 1}, \dots, c_{p + 1, k_{p + 1}} \in \mathbb{C}$, \dots, $v_{p + m, 1}, \dots, v_{p + m, k_{p + m}} \in B_{p + m}$, $c_{p + m, 1}, \dots, c_{p + m, k_{p + m}} \in \mathbb{C}$ such that $v - (c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{p + m, 1}v_{p + m, 1} + \dots + c_{p + m, k_{p + m}}v_{p + m, k_{p + m}}) \in F_{p + m + 1}M_{h + n}$.
  By \Cref{prp:17}(i) and \Cref{prp:19}, $F_{p + m + 1}M_{h + n} = 0$ for $m \ge n - p$.
  Hence, $v = c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{p + m, 1}v_{p + m, 1} + \dots + c_{p + m, k_{p + m}}v_{p + m, k_{p + m}}$ if $m$ is such that $m \ge n - p$, and $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$.
  If $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$, then clearly $\{v + F_{p + 1}M \mid v \in B_p\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.

  Finally, we prove that $\{v + F_{p + 1}M \mid v \in B_p\}$ is a basis of $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$ if and only if $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B_k$ is linearly independent.
  We now assume that $\{v + F_{p + 1}M \mid v \in B_p\}$ is a basis of $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.
  By the last paragraph, $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$.
  Let $n \in \mathbb{N}$.
  If $c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0} + \dots + c_{n, 1}v_{n, 1} + \dots + c_{n, k_n}v_{n, k_n} = 0$, where $v_{0, 1}, \dots, v_{0, k_0} \in B_0$, $c_{0, 1}, \dots, c_{0, k_0} \in \mathbb{C}$, \dots, $v_{n, 1}, \dots, v_{n, k_n} \in B_n$, $c_{n, 1}, \dots, c_{n, k_n} \in \mathbb{C}$, then $c_{0, 1}(v_{0, 1} + F_1M) + \dots + c_{0, k_0}(v_{0, k_0} + F_1M) = 0$.
  By the linear independence of $\{v + F_1M \mid v \in B_0\}$, $c_{0, 1} = \dots = c_{0, k_0} = 0$.
  Continuing this way, $c_{1, 1} = \dots = c_{1, k_1} = \dots = c_{n, 1} = \dots = c_{n, k_n} = 0$, and $\bigcup_{k \in \mathbb{N}}B^k$ is linearly independent.
  To end this proof, we assume that $\bigcup_{k \ge p}B_k$ spans $F_pM$ for $p \in \mathbb{N}$, and $\bigcup_{k \in \mathbb{N}}B_k$ is linearly independent.
  By the last paragraph, $\{v + F_{p + 1}M \mid v \in B_p\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  Let $v_{p, 1}, \dots, v_{p, k_p} \in B_p$ and $c_{p, 1}, \dots, c_{p, k_p} \in \mathbb{C}$ be such that $c_{p, 1}(v_{p, 1} + F_{p + 1}M) + \dots + c_{p, k_p}(v_{p, k_p} + F_{p + 1}M) = 0$.
  Then $c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} \in F_{p + 1}M$, and $c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} = c_{p + 1, 1}v_{p + 1, 1} + \dots +  c_{p + 1, k_{p + 1}}v_{p + 1, k_{p + 1}} + \dots + c_{p + m, 1}v_{p + m, 1} + \dots + c_{p + m, k_{p + m}}v_{p + m, k_{p + m}}$ for some $m \in \mathbb{Z}_+$, $v_{p + 1, 1}, \dots, v_{p + 1, k_{p + 1}} \in B_{p + 1}$, $c_{p + 1, 1}, \dots, c_{p + 1, k_{p + 1}} \in \mathbb{C}$, \dots, $v_{p + m, 1}, \dots, v_{p + m, k_{p + m}} \in B_{p + m}$, $c_{p + m, 1}, \dots, c_{p + m, k_{p + m}} \in \mathbb{C}$ because $\bigcup_{k \ge p + 1}B_k$ spans $F_{p + 1}M$.
  By the linear independence of $\bigcup_{k \in \mathbb{N}}B_k$, $c_{p, 1} = \dots = c_{p, k_p} = 0$, and $\{v + F_{p + 1}M \mid v \in B_p\}$ is linearly independent.
\end{proof}
...
\begin{remark}
  \label{rmk:46}
  For a generalization of \Cref{thr:56} assuming $V$ is a lower truncated $\mathbb{Z}$-graded vertex algebra, see \cite[Theorem 4.8]{li_abelianizing_2005}.
\end{remark}
...
\begin{remark}
  ...
  From this example, we see that the assumption of $V$ being $\mathbb{N}$-graded cannot be removed in \Cref{thr:56}, although we can assume that $V$ is lower truncated $\mathbb{Z}$-graded.
\end{remark}
...
\begin{proof}\leavevmode
  ...
  \item[(ii)$\implies$(iv)] By \Cref{thr:63}, $\dim(JX_V) = 0$.
...
\begin{example}
  \label{exa:20}
  By \Cref{exa:12}, \Cref{exa:16} and \Cref{thr:58}, $\Vir_{p, q}$ is regular for $p, q \ge 2$ relatively prime integers.
\end{example}
...
\begin{example}[$R_{M(c, h)}$ and $R_{L(c, h)}$]
  ...
  By \Cref{prp:19}, \Cref{exa:15} and \Cref{exa:16}, we have the following isomorphism of modules over Poisson algebras
  \begin{align*}
...
\begin{proof}
  The assertion follows from \Cref{lmm:26} and \Cref{thr:59}.
\end{proof}
...
\begin{example}[$JR_{\Vir^c} \otimes_{R_{\Vir^c}} R_{M(c, h)}$]
  \label{exa:22}
  We pick a highest weight $(c, h)$.
  The Verma module $M(c, h)$ is always classically free as a $\Vir^c$-module because by \Cref{prp:19}, \Cref{exa:15}, \Cref{exa:16} and \Cref{exa:21},
  \begin{align*}
...
\begin{proof}
  The proof of \Cref{thr:57} works with minor modifications.
\end{proof}
...
By \Cref{thr:62}, when $V$ is a $C_2$-cofinite conical conformal vertex algebra, to find all irreducibles in the category of $V$-modules, it is enough to find all irreducibles in the category of admissible $V$-modules.
...
\begin{proof}
  ...
  By \Cref{lmm:35} and \eqref{eq:53}, we have $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}
...
By \Cref{lmm:36}, \Cref{rmk:50} and the fact that $K$ is a vertex Poisson submodule (not just a submodule), we have
\begin{equation*}
  K' \subseteq K.
\end{equation*}
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  We have to keep in mind the isomorphisms given in \Cref{prp:22}.
  ...
  Therefore, by \Cref{lmm:33}, isomorphism \eqref{eq:54} and the isomorphism between $\gr^G(L)_{1/2 + n}$ and $L_{1/2 + n}$ of \Cref{prp:22}, we have
  \begin{equation*}
  ...
  By \Cref{lmm:37}, there would exist $u \in K' \cap F_N \subseteq K \cap F_N$ such that $\lm(u) = u_{\eta}$.
  ...
  This observation, together with the isomorphism between $L$ and $\gr^G(L)$ of \Cref{prp:22}, concludes the proof of \Cref{thr:4}.
\end{proof}
