...
\begin{example}[\emph{Sugawara construction} {\cite[Theorem 5.7]{kac_vertex_1998}}]
  ...
  \end{equation*}
  Then $\omega$ is a conformal vector with central charge $\frac{k\sdim(\mathfrak{g})}{k + h^{\wedge}}$, and for $a \in \mathfrak{g}$, $a(z)$ is a primary eigendistribution of conformal weight $1$.
\end{example}
...
\begin{remark}
  \label{rmk:23}
  We can define in a natural way the categories \{smooth $(\mathfrak{g}, \mathfrak{F}, T_0)$-modules\} and \{smooth $(\mathfrak{g}, \mathfrak{F}, T_0)$-modules where $\mathfrak{h}$ acts as $\lambda$\}.
...
\begin{proof}
  ...
  \begin{enumerate}
  \item From the formula $[L_0, a_n] = -na_n$ given in \eqref{eq:27}, we inductively get the formula
    \begin{equation*}
...
\end{enumerate}
The ordinary $V$-modules form a subcategory of $V$-Mod, and they satisfy \eqref{eq:29}.
...
\begin{example}
  ...
  By \Cref{thr:25} and \Cref{thr:27}, $M$ is a $h + \mathbb{N}$-graded $(\Vir^c, L_{-2}\vac)$-module.
...
By \Cref{lmm:18}(iii), $M(c, h)$ has a unique maximal subrepresentation, denoted by $J(c, h)$.
The quotient
\begin{equation*}
  L(c, h) = M(c, h)/J(c, h)
\end{equation*}
...
We note, however, that initially we cannot define the form using \eqref{eq:36} for a general highest weight representation because there may be linear dependences between terms of the form $L_{\lambda}v$ for partitions $\lambda$, so we may ask if it is well defined in the first place.
...
\begin{theorem}[{\cite[Proposition 3.4]{kac_bombay_2013}}]
  ...
  \item Let $V$ be a highest weight representation with highest weight vector $v'$.
    There is a unique contravariant form $\langle\bullet| \bullet\rangle$ on $V$ such that $\langle v'| v'\rangle = 1$ and is defined by equation \eqref{eq:36} with $v'$ in place of $v$.
...
We now define
\begin{align*}
  J &= \{u \in M(U) \mid \text{for }u' \in U^*\text{ and }x \in U([V]), \langle u', xu\rangle = 0\}, \\
  L(U) &= M(U)/J,
\end{align*}
...
Given a vertex Lie superalgebra $V$, we obtain the abelian category $\text{$V$-Mod}$ of modules over $V$.
...
Given a vertex Poisson algebra $V$, we obtain the abelian category $\text{$V$-Mod}$ of modules over $V$.
...
\begin{remark}
  \label{rmk:48}
  By construction and \Cref{rmk:47}, for each pivot $\lambda$ of $A^W_n$, we have $u^W_{\lambda} \in W$, $u^K_{\lambda} \in K$ and $\lm(u^K_{\lambda}) = u_{\lambda}$.
\end{remark}
...
By \Cref{lmm:36}, \Cref{rmk:48} and the fact that $K$ is a vertex Poisson submodule (not just a submodule), we have
\begin{equation*}
K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  We pick $v \in K'$ such that $\lm(v) = u_{\eta}$ and some power product $p_{\tau} \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_{\tau}v)) = u_{\lambda}$ (this can be done because of \Cref{rmk:49}, and the fact that $\lm$ is multiplicative).
...
\begin{proof}
  ...
  By \Cref{rmk:50}, $u_{[\eta, 1, \dots, 1]} \mid u_{\lambda}$ where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:50}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $u_{\eta} \nmid u_{\lambda}$.
