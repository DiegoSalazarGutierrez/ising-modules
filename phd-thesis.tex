...
A module $M$ over a vertex superalgebra $V$ can be defined as a vector superspace $M$ together with a linear and parity preserving map $Y^M: V \to \End(M)[[z^{\pm 1}]]$ satisfying the vacuum axiom and the Borcherds identity.
...
To describe the irreducible modules over $\Vir_{3, 4}$, we study filtrations of vertex algebras and their modules.
...
A \emph{Virasoro formal distribution} with central charge $C$ is a $\mathfrak{g}$-valued formal distribution $L(z) \in \mathfrak{g}[[z^{\pm 1}]]$ satisfying \eqref{eq:6} or, equivalently, \eqref{eq:7}, \eqref{eq:8} or \eqref{eq:9}.
...
\begin{lemma}
  \label{lmm:3}
  Consider $s$ fields $a^1(z), \dots, a^s(z) \in \mathcal{F}(V)$. with $s \ge 2$. and let $b \in V$.
  For $l \in \mathbb{Z}$,
  \begin{equation*}
...
\begin{theorem}[{\cite[Proposition 3.4.3]{nozaradan_introduction_2008}}]
  ...
  \begin{equation*}
    a(w)_{(j)}b(w)c = \Res_z(i_{z, w}((z - w)^j)a(z)b(w)c - p(a, b)i_{w, z}((z - w)^j)b(w)a(z)c) \quad \text{for }c \in V.
  \end{equation*}
\end{theorem}
...
\begin{example}[\emph{Commutative vertex algebras}]
  ...
  Conversely, let $V$ be a differential commutative associative algebra with unit $1$, and let $T$ be a derivation of $V$.
...
\begin{remark}
  ...
  \begin{equation*}
    \begin{tikzcd}
      {V(\mathfrak{g}, \mathfrak{F}, T_0)} \arrow[rd, "f"'] \arrow[r, "\pi", two heads] & {V^{\lambda}(\mathfrak{g}, \mathfrak{F}, T_0)} \arrow[d, "\bar{f}"] \\
      & W
    \end{tikzcd}
  \end{equation*}
\end{remark}
...
Let $V$ be a vertex superalgebra.
We can naturally endow $V$ with the structure of a Lie conformal superalgebra by taking $\partial = T$, and by defining the following $\lambda$-bracket on $V$ as
\begin{align*}
  [\bullet_{\lambda}\bullet]: V \times V &\to V[\lambda], \\
...
\begin{remark}
  ...
  We have two meanings of the word homogeneous, one related to the Hamiltonian, and the other related to parity.
  Context will clarify the meaning.
\end{remark}
...
An ideal $I$ of a graded vertex superalgebra $V$ is called \emph{graded} if it is $H$-invariant or, equivalently, $I = \sum_{\Delta \in \mathbb{C}}I \cap V_{\Delta}$ (see \Cref{lmm:9}).
...
Condition \eqref{eq:25} is equivalent to
\begin{equation}
  \label{eq:26}
...
Thus, we have a functor defined by
\begin{align*}
  \Shift: \{\text{admissible $V$-modules}\} &\to \{\text{admissible $V$-modules}\}, \\
  \Shift\left(M, \bigoplus_{n \in \mathbb{Z}}M(n)\right) &= \left(M, \bigoplus_{n \in \mathbb{Z}}M(n - n_0)\right),
\end{align*}
if $M \neq 0$ and $M(n_0)$ is the top degree component of $M$.
We set $\Shift(0) = 0$.
This functor is clearly an equivalence of categories.
...
\begin{proof}
  ...
  \begin{enumerate}
  \item From the formula $[L_0, a_n] = -na_n$ given in \eqref{eq:20}, we inductively get the formula
    \begin{equation*}
...
\end{enumerate}
The ordinary $V$-modules form a subcategory of $V\text{-Mod}$, and they satisfy \eqref{eq:30}.
...
\begin{example}
  ...
  Furthermore, $V$ is an ordinary positive energy representation of $\Vir^c$.
\end{example}
...
We assume now that $V$ is a $\mathbb{Z}$-graded vertex algebra with Hamiltonian $H$.
...
We note, however, that initially we cannot define the form using \eqref{eq:37} for a general highest weight representation because there may be linear dependences between terms of the form $L_{\lambda}v$ for a partition $\lambda$, so we may ask if it is well defined in the first place.
...
\begin{theorem}[{\cite[Proposition 3.4]{kac_bombay_2013}}]
  ...
  \item Every highest weight representation $V$ of $\Vir$ with highest weight $(c, h)$ and highest weight vector $v'$ carries a contravariant form such that $\langle v'| v'\rangle = 1$.
    This form on $V$ is defined by \eqref{eq:37} with $v'$ in place of $v$ and satisfies properties \emph{(i)--(iii)} of this Theorem.
  ...
  \item Let $V$ be a highest weight representation with highest weight vector $v$.
    There is a unique contravariant form $\langle\bullet| \bullet\rangle$ on $V$ such that $\langle v| v\rangle = 1$ and is defined by equation \eqref{eq:37}.
...
\begin{remark}
  ...
  If $c = c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, then the singular vector of $\Vir^c$ can be computed by solving the linear equation $L_1u = L_2u = 0$ for $u \in \Vir^c_{(p - 1)(q - 1)}$ (see \Cref{thr:23} and \Cref{lmm:19}).
\end{remark}
...
\begin{remark}
  ...
  Moreover, $L(1/2, 1/2)$ is isomorphic to $F_{\one}$ where $F = F(\mathbb{C}a)$ is the fermionic vertex algebra associated to a purely odd one dimensional superspace $\mathbb{C}a$ with an antisupersymmetric form defined by $\langle a| a\rangle = 1$ (see \Cref{exa:10}).
\end{remark}
...
\end{align*}
Extend these operations linearly to obtain bilinear products on $V$, and we define
\begin{equation*}
...
\end{equation*}
However, we are not done yet because all the conditions in \Cref{prp:9} are met except \eqref{eq:24}.
...
\begin{remark}
  ...
  This implies that $L(U)$ is strongly generated by $U$ as a vertex algebra if $U$ is simple.
\end{remark}
...
\begin{proof}
  ...
  Also, $L_0 = (L_{-2}\vac)^{L(\mathbb{C}_h)}_{(1)}$ acts by multiplication by $h$ on $\mathbb{C}_h$ and by \Cref{rmk:34}, $L(\mathbb{C}_h)$ is strongly generated by $\mathbb{C}_h$ as a vertex algebra.
...
We obtain the abelian category of vertex Lie superalgebras.
...
A \emph{module} over a vertex Lie superalgebra $V$ is a vector superspace $M$ together with a linear and parity preserving map $Y^M_-: V \to \Hom(M, z^{-1}M[z^{-1}])$, written as $Y^M_-(a, z) = \sum_{n \in \mathbb{N}}a^M_{(n)}z^{-n - 1}$, $a^M_{(n)} \in \End(M)$, such that for $a, b \in V$ and $m, n \in \mathbb{N}$:
\begin{enumerate}
...
We obtain the abelian category of vertex Poisson algebras.

A \emph{module} over a vertex Poisson algebra $(V, T, Y_-)$ is a module $(M, Y^M_-)$ over $V$ as a vertex Lie algebra and a module over $V$ as a commutative associative algebra such that
\begin{equation*}
...
\begin{equation*}
  \begin{split}
    F_pM = \vspan\{a^{1M}_{(-n_1 - 1)}\dots a^{sM}_{(-n_s - 1)}u &\mid s, n_1, \dots, n_s \in \mathbb{N}, a^i \in V\text{ homogeneous}, \\
...
\begin{proof}\leavevmode
  ...
  \item We assume that $M$ is an irreducible admissible $V$-module.
    By (ii), we can assume that $M = \bigoplus_{n \in \mathbb{Z}}M^{\Gen}(n)$.
    Let $N$ be a proper submodule de $M$.
...
In this section, our objective is to determine a monomial basis (also called PBW basis) of $L(1/2, 1/2)$ explicitly and a formula for the refined character of $\gr^G(L(1/2, 1/2))$ with respect to the standard filtration, as introduced in \Cref{sec:filtr-modul-over}.
...
We now define subsets of $P$ which will help us in finding an expression for $p(t, q)$.
We set:
\begin{align*}
...
\end{align*}
where $a_{3, 4}$ is the singular vector of $V$ that generates its maximal subrepresentation as in \cite{andrews_singular_2022} and \eqref{eq:21}.
...
\begin{proof}
  ...
  By \Cref{lmm:35} and \eqref{eq:49}, $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}
...
For $n \in \mathbb{N}$, we compute the matrix $A_n$ with $p(n - 2) + p(n - 3)$ rows and $p(n)$ columns, which is given by stacking the matrix $A^{m_3}_n$ below the matrix $A^{m_2}_n$, that are described next.
...
The \emph{universal enveloping algebra} $U(\mathfrak{g}) = T(\mathfrak{g})/(xy - yx - [x, y] \mid x, y \in \mathfrak{g})$ of $\mathfrak{g}$ is also an associative algebra with $1$, but we also have an inclusion $\inc: \mathfrak{g} \hookrightarrow U(\mathfrak{g})$, which is a Lie algebra homomorphism.
...
The \emph{symmetric algebra} $S(\mathfrak{g}) = T(\mathfrak{g})/(xy - yx \mid x, y \in \mathfrak{g})$ of $\mathfrak{g}$ is a commutative associative algebra with unit $1$, and we have an inclusion $\inc: \mathfrak{g} \hookrightarrow S(\mathfrak{g})$ satisfying an universal property.
...
\begin{example}
  ...
  We know that $\gr(A)$ is an associative commutative algebra with unit $\gamma^0(1)$.
