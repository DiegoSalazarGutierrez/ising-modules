...
Specializing the character formula in \Cref{thr:8} to $t = 1$, we obtain that the character of $L(1/2, 1/2)$ is the sum of three Nahm sums for the same matrix $\left(\begin{smallmatrix} 8 & 3 \\ 3 & 2 \end{smallmatrix}\right)$ (cf.\ \cite{Nahm2007}).
...
This thesis is organized as follows: In \Cref{sec:vert-algebr-their}, we introduce the notion of vertex algebras and their modules while trying to compile several well-known results.
...
In \Cref{sec:modules-over-simple}, we quickly review the theory of the $\Zhu(V)$ algebra to use it to prove the theorems at hand.
In \Cref{sec:vert-poiss-algebr}, the theory of filtrations of vertex algebras and their modules started by Haisheng Li is presented, we introduce the standard filtration for modules and prove it is compatible with the definitions already given by Li.
...
By \Cref{thr:16}, we can define a linear map
\begin{align*}
...
\begin{proof}\leavevmode
  ...
  \item By definition, we have
    ...
    \end{equation*}
    and the right hand side, by \Cref{thr:13}, is equal to
    \begin{equation*}
  ...
  \item Set $b_1(z, w) = i_{z, w}a(z + w)$ and $b_2(z, w) = e^{Tw}a(z)e^{-Tw}$.
    By \Cref{thr:7} and translation covariance,
    \begin{align*}
  ...
  \item By locality, there is $N \in \mathbb{Z}$ such that
    ...
    \end{equation*}
    Apply $\vac$ to both sides; by \Cref{thr:16}, we get
    \begin{equation*}
      (z - w)^Na(z)e^{Tw}b = p(a, b)(z - w)^Nb(w)e^{Tz}a.
    \end{equation*}
    Now use (f) and \Cref{thr:7}:
    \begin{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  By the property (i), we have $a_{(-1)}\vac = 0$ and $a(z)$ is translation covariant, hence by \Cref{thr:16}(i), $b(w)\vac \in V[[w]]$, so we can let $w = 0$ and get $z^Na(z)b = 0$, which means $a_{(n)}b = 0$ for any $n \in \mathbb{Z}$.
...
\begin{remark}
  \label{rmk:8}
  The translation covariance axiom together with \Cref{thr:16} permit us to express $T$ by
  \begin{equation}
    \label{eq:15}
    Ta = a_{(-2)}\vac.
  \end{equation}
...
\begin{proof}
  By \Cref{thr:16}, the map $\fs$ is well defined as was noted already in \Cref{sec:fields-over-vector}.
  By \Cref{thr:17}(i) and \Cref{thr:17}(iii), $\fs|_{\mathcal{F}_{\min}}: \mathcal{F}_{\mathcal{F}_{\min}} \to V$ is given by
  \begin{equation}
    \label{eq:17}
...
By \Cref{thr:21} and the Extension theorem, $V$ is a vertex algebra, denoted by $V(\mathfrak{g}, \mathfrak{F}, T_0)$, and by the PBW theorem, it is explicitly given by
\begin{equation}
  \label{eq:20}
...
By \Cref{thr:21}, $\{a^j_{(-1)}\vac \}_{j \in J}$ strongly generates $V$ and from this we see that $I^{\lambda}$ is a left ideal.
...
\begin{proof}
  By \Cref{thr:18}(iii) and \Cref{thr:18}(v), for $a \in V$ homogeneous and $n \in \mathbb{Z}$,
  \begin{align*}
  ...
  By the $n$-product identity for vertex algebras and \Cref{thr:18}(iii), for $a \in V$,
  \begin{align*}
...
\begin{proof}
  ...
  By \Cref{rmk:15}, we obtain a homomorphism
  \begin{align*}
    f: VA(\Vir) &\to V \\
    f(L_{-2}\vac) &= \omega \\
    f(C) &= c\vac.
  ...
  Alternatively, we could use \Cref{thr:27} ahead to obtain a state-field correspondence $f: \Vir^c \to \mathcal{F}(V)$.
...
An ideal $I$ of a graded vertex algebra $V$ is called graded if $H(I) \subseteq I$ or, equivalently, $I = \bigoplus_{\Delta \in \mathbb{C}}V_{\Delta} \cap I$ (see \Cref{lmm:9}).
...
\begin{proof}
  ...
  As $\omega_{(1)}(I) \subseteq I$, $I$ is a graded ideal by \Cref{lmm:9}.
...
\begin{theorem}[{\cite{gorelik_simplicity_2007}}]
  ...
  \end{enumerate}
  Moreover, the maximal ideal of $\Vir^{c_{p, q}}$ is generated by a singular vector of conformal weight $(p - 1)(q - 1)$ (see \Cref{sec:high-weight-repr} ahead).
\end{theorem}

In this thesis, we mainly deal with the Ising model $\Vir_{3, 4} = \Vir_{c_{3, 4}} = \Vir_{1/2}$ and its modules.
By \Cref{thr:25}, its maximal ideal is generated by the singular vector
\begin{equation}
  \label{eq:24}
...
Most of the theorems about vertex algebras and their proofs carry over to modules over vertex algebras (cf. \Cref{thr:18}) as the following theorem shows.
...
\begin{remark}
  \label{rmk:18}
  A $V$-module is equivalently a vector space $M$ such that (i), (v) and (viii) in \Cref{thr:26} hold.
\end{remark}
...
\begin{remark}
  ...
  \Cref{thr:27} constructs a pair of functors
  \begin{align*}
...
\begin{proof}
  The proof for \Cref{thr:22} also works here.
\end{proof}
...
\begin{remark}
  \label{rmk:21}
  \Cref{thr:29} is analogous to \Cref{thr:22} with $L_{-1}^M$ in place of $T$ and $L_0^M$ in place of $H$.
\end{remark}

Let $V$ be a conformal vertex algebra with conformal vector $\omega$.
It is better to use the notation $(V, \omega)$ for now because of \Cref{rmk:17}
By \Cref{thr:29}, the following definition makes sense:
...
\begin{example}
  ...
  By \Cref{thr:27} and \Cref{thr:28}, $V$ is a graded $\Vir^c$-module.
  Set $V(n) = V_{h + n}$ for $n \in \mathbb{N}$.
  By \Cref{thr:28}(ii), $V = \bigoplus_{n \in \mathbb{N}}V(n)$ is an admissible $\Vir^c$-module with top degree component $\mathbb{C}v$.
\end{example}
...
\begin{proof}
  ...
  The Jacobi identity follows from \Cref{thr:18}(x) with $m = n = 0$.
\end{proof}
...
\begin{proof}
  First, this map is well defined because of \Cref{thr:26}(ii).
  It is a Lie algebra homomorphism because of \Cref{thr:26}(viii).
\end{proof}
...
By \eqref{eq:28} and \Cref{thr:22}(vi), the bracket in $\Lie(V)_0$ is given by
\begin{equation}
  \label{eq:29}
...
\begin{proof}
  Let $U$ be a subrepresentation of $V$.
  By \Cref{thr:30}, $U^{\perp}$ is a subrepresentation of $V$ and $U\oplus U^{\perp} = V$.
...
But $\langle v| \omega(P)u\rangle = \langle \omega(P)u\rangle$ because all eigenspaces of $L_0$ are pairwise orthogonal by \Cref{thr:30}.
...
By \Cref{thr:31}, this is independent of the choice of the highest weight vector $v$ and there is essentially one form.
...
\begin{remark}
  \label{rmk:25}
  \Cref{thr:34} gives us an algorithm to find $J(c, h)$ for a given highest weight $(c, h)$.
  We merely need to find the levels at which the singular vectors that generate $J(c, h)$ lie and then by \Cref{lmm:17}, we have to solve the equation $L_1u = L_2 u = 0$ assuming $u$ lies in the right level to obtain our desired singular vector.
\end{remark}

\begin{remark}
  \label{rmk:26}
  For $c \in \mathbb{C}$, $\Vir^c = M(c, 0)/U(\Vir)\{L_{-1}\vac\}$ and $\Vir_c = L(c, 0)$ because by \Cref{rmk:20}, the ideals of $\Vir^c$ are in bijection with the submodules of $M(c, 0)/U(\Vir)\{L_{-1}\vac\}$.
  If $c = c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, then the singular vector of $\Vir^c$ can be computed by solving the equation $L_1u = L_2u = 0$ for $u \in \Vir^c_{(p - 1)(q - 1)}$ (see \Cref{thr:25} and \Cref{lmm:17})
\end{remark}
...
Next, we construct an inverse $L: \Zhu(V)\text{-mod} \to \{\text{admissible $V$-modules}\}$ of the functor $\Omega$ using the associated Lie algebras of a vertex algebra of \Cref{sec:lie-algebr-assoc}.
...
By \Cref{thr:36}(i)-(iii), we define the associative unital algebra
\begin{equation*}
  \Zhu(V) = V/J(V)
\end{equation*}
...
By \Cref{thr:36}(iv)-(v), we have a natural Lie algebra epimorphism $\Lie_0(V) \twoheadrightarrow [\Zhu(V)]$.
...
\begin{proof}
  ...
  By \Cref{thr:38} and \Cref{thr:39}, the simple modules of $\Vir^c$ are $L(\mathbb{C}_h)$ for $h \in \mathbb{C}$.
  By \Cref{thr:29}, for $h \in \mathbb{C}$, $L(\mathbb{C}_h)$ is a smooth $\Vir$-module with central charge $c$.
...
\begin{proof}
  ...
  We conclude from \Cref{thr:40} that the $\Vir_c$-module corresponding to $\mathbb{C}_{h_{m, n}}$ is $L(c, h_{m, n})$ and the commutativity of the diagram follows from \Cref{rmk:28}.
\end{proof}

\begin{remark}
  \label{rmk:29}
  It follows from \Cref{thr:42} that the irreducible modules of the Ising module $\Vir_{3, 4}$ are $\Vir_{3, 4}$, $L(1/2, 1/2)$ and $L(1/2, 1/16)$.
\end{remark}
...
\begin{proof}
  ...
  \end{equation}
  We are in Subsubcase III$_-$ of \Cref{thr:35} and we see that the first two elements of the solutions in \eqref{eq:41} are $(n, m)$ and $(n - q, m - p)$ (not necessarily in that order), so we are done.
\end{proof}
...
\begin{proof}\leavevmode
  ...
  \item This is \Cref{thr:11}. \qedhere
  \end{enumerate}
\end{proof}
...
By \Cref{lmm:19}, the Li filtration depends only on $V$ and not on the choice of the strong generators.
...
\begin{proof}
  The proof for \Cref{thr:45} also works here.
\end{proof}
...
By \Cref{lmm:20}, the Li filtration depends only on $M$ and not on the choice of the strong generators of $V$.
...
\begin{proof}
  ...
  Therefore, by \Cref{prp:11}(v), $a^{i_1M}_{(-n_1 - 1)}m \in G^{\Delta_{a^{i_1}} + \Delta_m - h - p + n_1}M = G^{n - p}M$.
...
\begin{example}[$\gr^G(M(c, h))$]
  ...
  By \Cref{thr:27}, the Verma module $M(c, h)$ is a module over $\Vir^c$ as a vertex algebra.
...
In this section, our objective is to determine a monomial basis for $L(1/2, 1/2)$ explicitly and a formula for the refined character of $\gr^G(L(1/2, 1/2))$ with respect to the standard filtration introduced in \Cref{sec:filtr-modul-over}.
...
The ordinary partitions are handled by the theory of $\Vir_{1/2}$-modules done in \Cref{sec:modules-over-simple}.
...
Finally, we give formal proofs of the theorems in \Cref{sec:introduction} of $L(1/2, 1/2)$.
...
\begin{proof}
  ...
  \end{align*}
  The solution to these equations is unique if it exists and we can verify using \Cref{lmm:21} that
  \begin{align*}
  ...
  We derive the formula $p_{>2}(t, q) = f_{3, 2, 0, 0}(t, q)$ again from \Cref{lmm:21}.
...
\begin{proof}
  ...
  \end{align*}
  The formula for $p_{>2, 1}(t, q)$ then follows from \Cref{lmm:21}.
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m, 1]
  \end{align*}
  from which together with the proof of \Cref{lmm:23} we get
  \begin{align*}
    p_{>4, 1, 1}(t, q) &= tqp_{>4, 1}(t, q) = f_{6, 4, 2, 2}(t, q), \\
    p_{4, 1, 1}(t, q) &= tqp_{>5, 4, 1}(t, q) = f_{9, 5, 6, 4}(t, q).
  \end{align*}
  The formula for $p_{>3, 1, 1}(t, q)$ then follows from \Cref{lmm:21}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  together with \Cref{lmm:22}, \Cref{lmm:23} and \Cref{lmm:24}.
\end{proof}
...
\begin{proof}
  This follows from \Cref{lmm:25} by setting $t = 1$ and \cite[Theorem 4]{andrews_singular_2022} together with \Cref{lmm:21}.
\end{proof}
...
\begin{proof}
  Follows immediately from \Cref{lmm:27}.
\end{proof}
...
\begin{proof}
  By \Cref{thr:42}, the following diagram commutes:
  \begin{equation*}
    \begin{tikzcd}
  ...
  By \Cref{lmm:28} and \eqref{eq:55}, $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}
...
\begin{remark}
  \label{rmk:37}
  By construction and \Cref{rmk:36}, for each pivot $\lambda$ of $A^W_n$, we have $u^W_\lambda \in W$, $u^K_\lambda \in K$ and $\lm(u^K_\lambda) = u_\lambda$.
\end{remark}
...
By \Cref{lmm:29}, \Cref{rmk:37} and the fact that $K$ is a Poisson submodule (not just a submodule), we have
\begin{equation*}
K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  Pick $v \in K'$ such that $\lm(v) = u_\eta$ and some power product $p_\tau \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_\tau v)) = u_\lambda$ (this can be done because of \Cref{rmk:38} and the fact that $\lm$ is multiplicative) so we set $u = (L_{-2})^k_{(0)}(p_\tau v) \in K'$ to get $\lm(u) = u_\lambda$.
...
\begin{proof}
  ...
  By \Cref{rmk:39}, $u_{[\eta, 1, \dots, 1]} \mid u_\lambda$ where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:39}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $u_\eta \nmid u_\lambda$.
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  Keep in mind the isomorphisms given in \Cref{prp:14}.
  ...
  Therefore, by \Cref{lmm:26}, isomorphism \eqref{eq:56} and the isomorphism between $\gr^G(L)_{1/2 + n}$ and $L_{1/2 + n}$ of \Cref{prp:14}, we have
  \begin{equation*}
  ...
  On the other hand, we have $B_N \subseteq \overline{B_N}$ because if $u_\lambda \in B_N$ and $\eta \in \overline{R}$ then we have $u_\eta \nmid u_\lambda$ because if we had $u_\eta \mid u_\lambda$, then by \Cref{lmm:30}, there would exist $u \in K' \cap F_N \subseteq K \cap F_N$ such that $\lm(u) = u_\eta$, then by the definition of Gr\"{o}bner basis, there would exist $v \in G_N$ such that $\lm(v) \mid \lm(u)$, implying $\lm(v) \mid u_\lambda$, a contradiction.
  As both $B_N$ and $\overline{B_N}$ are finite sets and $|B_N| = |\overline{B_N}|$, we get $B_N = \overline{B_N}$.
  We see that the family of bases given by \eqref{eq:57} satisfy the property that one basis is contained in the next one and this concludes the proof of \Cref{thr:4}.
\end{proof}

\begin{proof}[Proof of \Cref{thr:3}]
  Note that the basis of \Cref{thr:4} is also a basis of $\gr^G(L)$ under the isomorphism between $\gr^G(L)$ and $L$ of \Cref{prp:14}.
  ...
  Therefore, $\ch_{\gr^G(L)}(t, q) = q^{1/2}p(t, q)$ and together with \Cref{lmm:25}, we conclude the proof of \Cref{thr:3}.
\end{proof}
...
\begin{proof}[Sketch of proof]
  ...
  \end{align*}
  is a solution to these equations.
  We derive the formula $p_{>2}(t, q) = f_{2, 2, 0, 0}(t, q)$ again from \Cref{lmm:21}.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m, 1]
  \end{align*}
  from which together with the proof of \Cref{lmm:33} we get
  \begin{align*}
    p_{>4, 1, 1}(t, q) &= tqp_{>4, 1}(t, q) = f_{6, 4, 2, 2}(t, q), \\
  ...
  \end{align*}
  The formula for $p_{>2, 1, 1}(t, q)$ then follows from \Cref{lmm:21}.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  from which together with the proof of \Cref{lmm:32} and \Cref{lmm:21} we get
  \begin{align*}
  ...
  \end{align*}
  The formula for $p_{>3, 1, 1, 1}(t, q)$ then follows from \Cref{lmm:21}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  together with \Cref{lmm:32}, \Cref{lmm:33}, \Cref{lmm:34}, \Cref{lmm:35} and \Cref{lmm:21}.
\end{proof}
