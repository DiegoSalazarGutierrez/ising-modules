...
\begin{proof}
  ...
  Alternatively, we could use \Cref{thr:20} ahead to obtain a state-field correspondence $f: \Vir^c \to \mathcal{F}(V)$.
...
Most of the theorems about vertex algebras and their proofs carry over to modules over vertex algebras (cf. \Cref{thr:12}) as the following theorem shows.
...
\begin{remark}
  \label{rmk:18}
  A $V$-module is equivalently a vector space $M$ such that (i), (v) and (viii) in \Cref{thr:19} hold.
\end{remark}
...
\begin{remark}
  ...
  \Cref{thr:20} constructs a pair of functors
  \begin{align*}
...
\begin{proof}
  The proof for \Cref{thr:16} also works here.
\end{proof}
...
\begin{remark}
  \label{rmk:21}
  \Cref{thr:22} is analogous to \Cref{thr:16} with $L_{-1}^M$ in place of $T$ and $L_0^M$ in place of $H$.
\end{remark}

Let $V$ be a conformal vertex algebra with conformal vector $\omega$.
By \Cref{thr:22}, the following definition makes sense.
...
\end{equation*}
Note that, for $n \in \mathbb{Z}$ and $j \in \mathbb{N}$,
\begin{equation}
  \label{eq:28}
  L_nV_{h + j}\subseteq V_{h + j - n}.
\end{equation}
...
Note that $C$ acts on $V$ as multiplication by $c$ because it commutes with every $L_n$ for $n \in \mathbb{Z}$.
...
\begin{proof}\leavevmode
  \begin{enumerate}
  \item This is Theorem \ref{lmm:6} with $H = L_0$.
...
\begin{equation*}
  \chi_V(q) \le \frac{q^h}{\varphi(q)},
\end{equation*}
where $\varphi(q) = \prod_{j \in \mathbb{N}}(1 - q^j)$.
...
Since all vectors of the form $L_{-i_k}\dots L_{-i_1}v$ with $i_k \ge \dots \ge i_1 > 0$ in a Verma representation $V$ of highest weight $(c, h)$ are linearly independent, it follows that they form a basis of $V$ and that there is a homomorphism from $V$ to any other highest weight representation of $\Vir$ with the same highest weight $(c, h)$ mapping a highest weight vector to a highest weight vector.
...
As in \eqref{eq:29}, the character of a Verma representation is given by
\begin{equation*}
  \ch_{M(c, h)}(q) = \frac{q^h}{\varphi(q)}.
\end{equation*}
...
By \Cref{lmm:7}(iii), $M(c, h)$ has a unique maximal subrepresentation $J(c, h)$.
The quotient
\begin{equation*}
  L(c, h) = M(c, h)/J(c, h)
\end{equation*}
