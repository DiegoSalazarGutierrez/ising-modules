...
\begin{remark}
  \label{rmk:5}
  Properties (iii) and (vi) of \Cref{thr:7} tell us that $\partial: \mathfrak{g}[[z^{\pm1}]] \to \mathfrak{g}[[z^{\pm1}]]$ acts as a derivation on the $j$-products and the $\lambda$-bracket.
\end{remark}
...
\begin{proof}
  ...
  The base case $s = 2$ follows from \Cref{lmm:3} and by replacing $n$ by $-n - 1 \in \mathbb{N}$ in the following computation
  \begin{align*}
...
\begin{example}[\emph{Universal Virasoro vertex algebra of central charge $c$}]
  ...
  By \Cref{rmk:13}, for $c \in \mathbb{C}$, a basis of $\Vir^c$ is given by
  \begin{equation*}
...
Most of the theorems about vertex superalgebras and their proofs carry over to modules over vertex superalgebras (cf.\ \Cref{thr:16} and \Cref{crl:1}), as the following theorem shows.
...
\begin{theorem}
  ...
  \item $\Delta_{a^M_{(n)}u} = \Delta_a + \Delta_u - n - 1$ for $a \in V$ homogeneous, $u \in M$ homogeneous and $n \in \mathbb{Z}$;
...
\begin{example}
  ...
  In fact, as we will see in \Cref{thr:44}, a complete list of all the irreducible modules over $\Vir_{p, q}$ is given in that article.
...
\begin{remark}
  ...
  If $c = c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, then the singular vector of $\Vir^c$ can be computed by solving the linear equation $L_1u = L_2u = 0$ for $u \in \Vir^c_{(p - 1)(q - 1)}$ (see \Cref{thr:23}, \Cref{lmm:19} and \Cref{thr:46} ahead).
\end{remark}
...
If $c$ is not of the form $c_{p, q}$ for some $p, q \ge 2$ relatively prime integers, then $\Vir_c = \Vir^c$ by \Cref{thr:23}, and we have already solved the problem.
...
We are also interested in the refined character of these irreducible modules, as defined in \Cref{sec:vert-poiss-algeb} ahead.
...
\begin{theorem}[{\cite[Proposition 6.3]{iohara_representation_2011}} and {\cite{zhu_modular_1996}}]
  \label{thr:48}
  Let us take integers $p, q, m, n$ as in \Cref{thr:47}.
  Then:
  \begin{align*}
...
\begin{remark}
  \label{rmk:38}
  We probably cannot deduce the right Leibniz rule from the left Leibniz rule (cf.\ \Cref{rmk:53})
\end{remark}

\begin{proposition}[{\cite[Proposition 2.3.1]{arakawa_remark_2012}} and {\Cref{sec:jet-algebras-jet}}]
  \label{prp:11}
...
The (graded) vertex Poisson algebra structure on $JR$ given in \Cref{prp:11} for a (graded) Poisson algebra $R$ will be called the \emph{level 0 vertex Poisson algebra structure of $JR$}.
...
\end{equation*}
and $H^{JR \otimes_R M}$ as in \Cref{sec:jet-algebras-jet}.

We have a natural (graded) inclusion $\inc: M \hookrightarrow JR \otimes_R M$, which is a (graded) $R$-module homomorphism.
The (graded) $JR$-module $M$ together with the (graded) $R$-module inclusion $\inc: M \hookrightarrow JR \otimes_R M$ satisfy a universal property similar to that of $\inc: R \hookrightarrow JR$ (see \Cref{sec:jet-algebras-jet}), as we now show.
...
\begin{equation*}
  \sigma_p(F_pV_{\Delta}) \cong F_pV_{\Delta}/F_{p + 1}V_{\Delta} \quad \text{for $p \in \mathbb{N}$ and $\Delta \in \mathbb{C}$}
\end{equation*}
...
By \Cref{prp:14}(vii), we can define an operator $H \in \End(\gr^G(V))$ as $H(\alpha^p(a)) = \alpha^p(Ha)$ for $p \in \mathbb{N}$ and $a \in G^pV$.
...
\begin{proof}
  The proof of \Cref{thr:51} also works here.
\end{proof}

We have the natural vector space isomorphisms
\begin{equation*}
  \alpha^p(G^pV_n) \cong G^pV_n/G^{p - 1}V_n \quad \text{for $p, n \in \mathbb{N}$}
\end{equation*}
and the refined grading
\begin{equation}
  \label{eq:42}
...
\begin{remark}
  \label{rmk:40}
  It is worth noting that we do not use the Poisson structure of $\gr(U(\Vir_{\le -2}))$ given in \Cref{exa:24}.
\end{remark}
...
\begin{equation}
  \label{eq:45}
  \sigma^M_p(F_pM_{\Delta}) \cong F_pM_{\Delta}/F_{p + 1}M_{\Delta} \quad \text{for $p \in \mathbb{N}$ and $\Delta \in \mathbb{C}$}
\end{equation}
and the refined grading
\begin{equation}
  \label{eq:46}
...
\end{equation}
By \eqref{eq:46}, when $\dim(M)_{\Delta} < \infty$ for $\Delta \in \mathbb{C}$, it is natural to define the refined character of $M$ with respect to the Li filtration as
\begin{equation*}
...
\begin{lemma}
  \label{lmm:25}
  Let $V$ be an $\mathbb{N}$-graded vertex algebra, and let $M$ be an $h + \mathbb{N}$-graded $V$-module.
  Then:
  \begin{enumerate}
  \item $F_pM_{h + n} = 0$ for $p, n \in \mathbb{N}$ with $p > n$;
  \item $\bigcap_{p \in \mathbb{N}}F_pM = 0$;
  \item If $\dim(M_{h + n}) < \infty$ for $n \in \mathbb{N}$, then $\ch_M(q) = \ch_{\gr_F(M)}(q) = \ch_{\gr_F(M)}(1, q)$.
  \end{enumerate}
\end{lemma}

\begin{proof}\leavevmode
  \begin{enumerate}
  \item We just saw we can use homogeneous elements to define $F_pM$.
    By \Cref{thr:26}(ii),
    \begin{equation*}
      \Delta_{a^{1M}_{(-n_1 - 1)}\dots a^{sM}_{(-n_s - 1)}u} = \Delta_{a^1} + \dots + \Delta_{a^s} + \Delta_u + n_1 + \dots + n_s \ge h + n_1 + \dots + n_s.
    \end{equation*}
    Thus, if $p, n \in \mathbb{N}$ with $p > n$, then the elements $a^{1M}_{(-n_1 - 1)}\dots a^{sM}_{(-n_s - 1)}u$ spanning $F_pM$ have conformal weight greater than $h + n$, and this implies that $F_pM_{h + n} = 0$.
  \item By \eqref{eq:44} and (i),
    \begin{equation*}
      \bigcap_{p \in \mathbb{N}}F_pM = \sum_{n \in \mathbb{N}}\left(\left(\bigcap_{p \in \mathbb{N}}F_p\right) \cap M_{h + n}\right) = \sum_{n \in \mathbb{N}}\left(\bigcap_{p \in \mathbb{N}}F_pM_{h + n}\right) = \sum_{n \in \mathbb{N}}0 = 0.
    \end{equation*}
  \item The latter equality follows from the definition of $\gr_F(M)_{\Delta}$ and $\ch_{\gr_F(M)}(t, q)$.
    To prove $\ch_M(q) = \ch_{\gr_F(M)}(q)$, we have to prove $\dim(M_{h + n}) = \dim(\gr_F(M)_{h + n})$ for $n \in \mathbb{N}$.
    By definition, $\gr_F(M)_{h + n} = \bigoplus_{p \in \mathbb{N}}\sigma^M_p(F_pM_{h + n})$ for $n \in \mathbb{N}$.
    By \eqref{eq:45}, \Cref{prp:16}(i), and (i), for $n \in \mathbb{N}$,
    \begin{align*}
      \dim(\gr_F(M)_{h + n}) &= \sum_{p \in \mathbb{N}}\dim(\sigma^M_p(F_pM_{h + n})) \\
                             &= \sum_{p = 0}^n(\dim(F_pM_{h + n}) - \dim(F_{p + 1}M_{h + n})) \\
                             &= \dim(F_0M_{h + n}) \\
                             &= \dim(M_{h + n}). \qedhere
    \end{align*}
  \end{enumerate}
...
\begin{proof}
  To begin with, we prove that $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$ if and only if $B_p$ spans $F_pM$ for $p \in \mathbb{N}$.
  We now assume that $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.
  Let $p, n \in \mathbb{N}$, and let $v \in F_pM_{h + n}$.
  By hypothesis, there is $v_{p, 1}, \dots, v_{p, k_p} \in B_p$ and $c_{p, 1}, \dots, c_{p, k_p} \in \mathbb{C}$ such that $v + F_{p + 1}M = c_{p, 1}(v_{p, 1} + F_{p + 1}M) + \dots + c_{p, k_p}(v_{p, k_p} + F_{p + 1}M)$.
  This means that $v - (c_{p, 1}v_{p, 1} + \dots +  c_{p, k_p}v_{p, k_p}) \in F_{p +1}M$.
  By applying the projection $M \twoheadrightarrow M_{h + n}$, we may assume that $v_{p, 1}, \dots ,v_{p, k_p} \in M_{h + n}$.
  Therefore $v - (c_{p, 1}v_{p, 1} + \dots +  c_{p, k_p}v_{p, k_p}) \in F_{p +1}M_{h + n}$.
  Continuing this way, we find for $m \in \mathbb{Z}_+$, $v_{p + 1, 1}, \dots, v_{p + 1, k_{p + 1}} \in B_{p + 1}$, $c_{p + 1, 1}, \dots, c_{p + 1, k_{p + 1}} \in \mathbb{C}$, \dots, $v_{p + m, 1}, \dots, v_{p + m, k_{p + m}} \in B_{p + m}$ and $c_{p + m, 1}, \dots, c_{p + m, k_{p + m}} \in \mathbb{C}$ such that $v - (c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{p + m, 1}v_{p + m, 1} + \dots + c_{p + m, k_{p + m}}v_{p + m, k_{p + m}}) \in F_{p + m + 1}M_{h + n}$.
  By \Cref{lmm:25}(i), $F_{p + m + 1}M_{h + n} = 0$ for $m \ge n - p$.
  Hence, $v = c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{p + m, 1}v_{p + m, 1} + \dots + c_{p + m, k_{p + m}}v_{p + m, k_{p + m}}$ if $m$ is such that $m \ge n - p$, and $B_p$ spans $F_pM$ for $p \in \mathbb{N}$.
  If $B_p$ spans $F_pM$ for $p \in \mathbb{N}$, then clearly $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.

  Finally, we prove that $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ is a basis of $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$ if and only if $B_p$ is a basis of $F_pM$ for $p \in \mathbb{N}$.
  We now assume that $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ is a basis of $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.
  By the last paragraph, $B_p$ spans $F_pM$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  If $c_1v_1 + \dots + c_kv_k + d_1w_1 + \dots + d_rw_r = 0 = 0$, where $v_1, \dots, v_k \in B_p \setminus B_{p + 1}$, $w_1, \dots, w_r \in B_{p + 1}$ and $c_1, \dots, c_k, d_1, \dots, d_r \in \mathbb{C}$, then $c_1(v_1 + F_{p + 1}M) + \dots + c_k(v_k + F_{p + 1}M) = 0$.
  By the linear independence of $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$, $c_1 = \dots = c_k = 0$.
  Continuing this way, $d_1 = \dots = d_r = 0$, and $B_p$ is linearly independent.
  To end this proof, we assume that $B_p$ is a basis of $F_pM$ for $p \in \mathbb{N}$.
  By the last paragraph, $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ spans $F_pM/F_{p + 1}M$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  Let $v_1, \dots, v_k \in B_p \setminus B_{p + 1}$ and $c_1, \dots, c_k \in \mathbb{C}$ be such that $c_1(v_1 + F_{p + 1}M) + \dots + c_k(v_k + F_{p + 1}M) = 0$.
  Then $c_1v_1 + \dots + c_kv_k \in F_{p + 1}M$, and $c_1v_1 + \dots + c_kv_k = d_1w_1 + \dots + d_rw_r$ for some for some $w_1, \dots, w_r \in B_{p + 1}$ and $d_1, \dots, d_r \in \mathbb{C}$ because $B_{p + 1}$ spans $F_{p + 1}M$.
  By the linear independence of $B_p$, $c_1 = \dots = c_k = 0$, and $\{v + F_{p + 1}M \mid v \in B_p \setminus B_{p + 1}\}$ is linearly independent.
\end{proof}
...
By \Cref{prp:20} ahead, the standard filtration does not depend on the choice of the strong generators of $V$.

By \Cref{prp:18}(vii), we can define an operator $H^M \in \End(\gr^G(M))$ as $H^M(\alpha_M^p(u)) = \alpha_M^p(L^M_0u)$ for $p \in \mathbb{N}$ and $u \in G^pM$.
...
\begin{equation}
  \label{eq:47}
  \alpha_M^p(G^pM_{h + n}) \cong G^pM_{h + n}/G^{p - 1}M_{h + n} \quad \text{for $p, n \in \mathbb{N}$}
\end{equation}
and the refined grading
\begin{equation}
  \label{eq:48}
...
\end{equation}
By \eqref{eq:48}, when $M$ is ordinary, which means $\dim(M_{h + n}) < \infty$ for $n \in \mathbb{N}$, it is natural to define the refined character of $M$ with respect to the standard filtration as
\begin{equation*}
...
We now state a general linear algebra proposition about bases of increasing filtrations of vector spaces.
The proposition applies in particular to the standard filtration $(G^pM)_{p \in \mathbb{Z}}$ of a module $M$ over a vertex algebra, and the proof is straightforward.

\begin{proposition}
  \label{prp:19}
  Let $M$ be a vector space, let $(M^p)_{p \in \mathbb{Z}}$ be a family of subspaces of $M$ satisfying properties \emph{(i)--(ii)} of \Cref{prp:18}, and let $(B^p)_{p \in \mathbb{N}}$ be an increasing family of sets satisfying $B^p \subseteq M^p$ for $p \in \mathbb{N}$.
  ...
  In particular, if $M = \bigcup_{p \in \mathbb{N}}M^p$ and $\{v + M^{p - 1} \mid p \in \mathbb{N}, v \in B^p \setminus B^{p - 1}\}$ is a basis of $\gr(M) = \bigoplus_{p \in \mathbb{N}}M^p/M^{p - 1}$, then $\bigcup_{p \in \mathbb{N}}B^p$ is a basis of $M$.
\end{proposition}

\begin{proof}
  To begin with, we prove that $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$ if and only if $B^p$ spans $M^p$ for $p \in \mathbb{N}$.
  We now assume that $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$, and let $v \in M^p$.
  By hypothesis, there is $v_{p, 1}, \dots, v_{p, k_p} \in B^p \setminus B^{p - 1}$ and $c_{p, 1}, \dots, c_{p, k_p} \in \mathbb{C}$ such that $v + M^{p - 1} = c_{p, 1}(v_{p, 1} + M^{p - 1}) + \dots + c_{p, k_p}(v_{p, k_p} + M^{p - 1})$.
  This means that $v - (c_{p, 1}v_{p, 1} + \dots +  c_{p, k_p}v_{p, k_p}) \in M^{p - 1}$.
  Continuing this way, we find $v_{p - 1, 1}, \dots, v_{p - 1, k_{p - 1}} \in B^{p - 1}$, $c_{p - 1, 1}, \dots, c_{p - 1, k_{p - 1}} \in \mathbb{C}$, \dots, $v_{0, 1}, \dots, v_{0, k_0} \in B^0$ and $c_{0, 1}, \dots, c_{0, k_0} \in \mathbb{C}$ such that $v - (c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0}) \in M^{-1} = 0$.
  Therefore, $v = c_{p, 1}v_{p, 1} + \dots + c_{p, k_p}v_{p, k_p} + \dots + c_{0, 1}v_{0, 1} + \dots + c_{0, k_0}v_{0, k_0}$, and $B^p$ spans $M^p$ for $p \in \mathbb{N}$.
  If $B^p$ spans $M^p$ for $p \in \mathbb{N}$, then clearly $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.

  Finally, we prove that $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ is a basis of $M^p/M^{p - 1}$ for $p \in \mathbb{N}$ if and only if $B^p$ is a basis of $M^p$ for $p \in \mathbb{N}$.
  We now assume that $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ is a basis of $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.
  By the last paragraph, $B^p$ spans $M^p$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  If $c_1v_1 + \dots + c_kv_k + d_1w_1 + \dots + d_rw_r = 0$, where $v_1, \dots, v_k \in B^p \setminus B^{p - 1}$, $w_1, \dots, w_r \in B^{p - 1}$ and $c_1, \dots, c_k, d_1, \dots, d_r \in \mathbb{C}$, then $c_1(v_1 + M^{p - 1}) + \dots + c_k(v_k + M^{p - 1}) = 0$.
  By the linear independence of $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$, $c_1 = \dots = c_k= 0$.
  Continuing this way, we prove $d_1 = \dots = d_r = 0$, and $B^p$ is linearly independent.
  To end this proof, we assume that $B^p$ is a basis of $M^p$ for $p \in \mathbb{N}$.
  By the last paragraph, $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ spans $M^p/M^{p - 1}$ for $p \in \mathbb{N}$.
  Let $p \in \mathbb{N}$.
  Let $v_1, \dots, v_k \in B^p$ and $c_1, \dots, c_k \in \mathbb{C}$ be such that $c_1(v_1 + M^{p - 1}) + \dots + c_k(v_k + M^{p - 1}) = 0$.
  Then $c_1v_1 + \dots + c_kv_k \in M^{p - 1}$, and $c_1v_1 + \dots + c_kv_k = d_1w_1 + \dots + d_rw_r$ for some $w_1, \dots, w_r \in B^{p - 1}$ and $d_1, \dots, d_r \in \mathbb{C}$ because $B^{p - 1}$ spans $M^{p - 1}$.
  By the linear independence of $B^p$, $c_1 = \dots = c_k = 0$, and $\{v + M^{p - 1} \mid v \in B^p \setminus B^{p - 1}\}$ is linearly independent.
\end{proof}
...
\begin{proof}
  ...
  Therefore, by \Cref{prp:18}(v), $a^{i_1M}_{(-n_1 - 1)}u \in G^{\Delta_{a^{i_1}} + \Delta_u - h - p + n_1}M = G^{n - p}M$.
...
\Cref{prp:20} translates into an identity of the refined characters of $M$, as the following proposition shows.
This is also valid for vertex algebras because all the definitions are compatible.

\begin{proposition}
  \label{prp:21}
  Let $(V, \omega)$ be an $\mathbb{N}$-graded conformal vertex algebra, and let $M$ be an ordinary $h + \mathbb{N}$-graded $(V, \omega)$-module.
  Then:
  \begin{enumerate}
  \item $\ch_M(q) = \ch_{\gr^G(M)}(q) = \ch_{\gr^G(M)}(1, q)$;
...
\begin{proof}\leavevmode
  \begin{enumerate}
  \item The latter equality follows from the definition of $\gr^G(M)_{h + n}$ and $\ch_{\gr^G(M)}(t, q)$.
    To prove $\ch_M(q) = \ch_{\gr^G(M)}(q)$, we have to prove $\dim(M_{h + n}) = \dim(\gr^G(M)_{h + n})$ for $n \in \mathbb{N}$.
    By definition, $\gr^G(M)_{h + n} = \bigoplus_{p \in \mathbb{N}}\alpha_M^p(G^pM_{h + n})$ for $n \in \mathbb{N}$.
    By \Cref{prp:18}(ii),(iv), $G^pM_{h + n} = M_{h + n}$ for $p, n \in \mathbb{N}$ with $p \ge n$.
    This observation together with \eqref{eq:47} implies that $\alpha_M^p(G^pM_{h + n}) = 0$ for $p, n \in \mathbb{N}$ with $p \ge n + 1$.
    By \Cref{prp:18}(i), $G^{-1}M_{h + n} = 0$ for $n \in \mathbb{N}$.
    Again by \eqref{eq:47}, $\dim(\alpha_M^p(G^pM_{h + n})) = \dim(G^pM_{h + n}) - \dim(G^{p - 1}M_{h + n})$ for $p, n \in \mathbb{N}$.
    We now do the summation.
    \begin{align*}
      \dim(\gr^G(M)_{h + n}) &= \sum_{p \in \mathbb{N}}\dim(\alpha_M^p(G^pM_{h + n})) \\
                             &= \sum_{p = 0}^n(\dim(G^pM_{h + n}) - \dim(G^{p - 1}M_{h + n})) \\
                             &= \dim(G^nM_{h + n}) \\
                             &= \dim(M_{h + n}).
    \end{align*}
  \item This follows from \Cref{prp:20} and by replacing $p$ by $n - p$ in the following computation.
    \begin{align*}
...
\begin{example}[$\gr^G(M(c, h))$]
  ...
  From \Cref{exa:14}, $\gr^G(\Vir^c) \cong \mathbb{C}[L_{-2}, L_{-3}, \dots]$.
...
By the universal property of $\inc: R \hookrightarrow JR_V$ (see \Cref{sec:jet-algebras-jet}), there is a (graded) differential algebra homomorphism $\phi_V: JR_V \to \gr_F(V)$ such that the following diagram commutes
\begin{equation*}
  \begin{tikzcd}
...
\begin{proof}
  The homomorphism $\phi_V$ is surjective by \Cref{lmm:26}.
...
\begin{example}[$JR_{\Vir^c}$ and $JR_{\Vir_c}$]
  ...
  \end{equation*}
  by \Cref{exa:14} and \Cref{prp:15} and
  \begin{equation*}
    JR_{\Vir^c} \cong J(C[L_{-2}]) = \mathbb{C}[L_{-2}, L_{-3}, \dots]
  \end{equation*}
  by \Cref{exa:16} and \Cref{sec:jet-algebras-jet}.
...
\begin{proof}
  ...
  By \eqref{eq:49}, we can write $b = \sum_{n \in \mathbb{N}}b_n$, where $b_n \in W \cap V_n$ for $n \in \mathbb{N}$.
...
\begin{remark}
  \label{rmk:46}
  For a generalization of \Cref{thr:55} assuming $V$ is a lower truncated $\mathbb{Z}$-graded vertex algebra, see \cite[Theorem 4.8]{li_abelianizing_2005}.
\end{remark}
...
\begin{remark}
  ...
  From this example, we see that the assumption of $V$ being $\mathbb{N}$-graded cannot be removed in \Cref{thr:55}, although we can assume that $V$ is lower truncated $\mathbb{Z}$-graded.
\end{remark}
...
\begin{proof}\leavevmode
  ...
  \item[(ii)$\implies$(iv)] By \Cref{thr:62}, $\dim(JX_V) = 0$.
    By \Cref{lmm:27}, $\dim(SS(V)) = 0$.
...
\begin{example}
  \label{exa:20}
  By \Cref{exa:12}, \Cref{exa:16} and \Cref{thr:57}, $\Vir_{p, q}$ is regular for $p, q \ge 2$ relatively prime integers.
\end{example}
...
If $V$ is graded and $M$ is a graded $V$-module, then, as we explained in \Cref{sec:filtr-modul-over}, $\gr_F(M)$ is a graded $\gr_F(V)$-module.
...
\begin{example}[$R_{M(c, h)}$ and $R_{L(c, h)}$]
  ...
  By \Cref{prp:20}, \Cref{exa:15} and \Cref{exa:16}, we have the following isomorphism of modules over Poisson algebras
  \begin{align*}
...
As we explained in \Cref{sec:vert-poiss-algeb-1}, we can consider $JR_V \otimes_{R_V} R_M$ as a (graded) $JR_V$-module.
...
By the universal property of $\inc: R_M \hookrightarrow JR_V \otimes_{R_V} R_M$ (see \Cref{sec:vert-poiss-algeb-1}), there is a (graded) $JR_V$-module homomorphism $\phi_M: JR_V \otimes_{R_V} R_M \to \gr_F(M)$ such that the following diagram commutes
\begin{equation*}
  \begin{tikzcd}
...
\begin{proof}
  The assertion follows from \Cref{lmm:27} and \Cref{thr:58}.
\end{proof}
...
\begin{example}[$JR_{\Vir^c} \otimes_{R_{\Vir^c}} R_{M(c, h)}$]
  \label{exa:22}
  We pick a highest weight $(c, h)$.
  The Verma module $M(c, h)$ is always classically free as a $\Vir^c$-module because by \Cref{prp:20}, \Cref{exa:15}, \Cref{exa:16} and \Cref{exa:21},
  \begin{align*}
...
\begin{proof}
  The proof of \Cref{thr:56} works with minor modifications.
\end{proof}
...
By \Cref{thr:61}, when $V$ is a $C_2$-cofinite conical conformal vertex algebra, to find all irreducibles in the category of $V$-modules, it is enough to find all irreducibles in the category of admissible $V$-modules.
...
As we can see, the arguments are similar to those in \cite{andrews_singular_2022}, but there are some differences such as the use of the Zhu algebra here but not in \cite{andrews_singular_2022}, and the fact that we do not have a differential available (see \Cref{rmk:41}).
...
\begin{proof}
  ...
  \end{align*}
  The solution to these equations is unique if it exists, and we can verify using \Cref{lmm:29} that:
  \begin{align*}
  ...
  We derive the formula $p_{>2}(t, q) = f_{3, 2, 0, 0}(t, q)$ again from \Cref{lmm:29}.
...
\begin{proof}
  ...
  \end{align*}
  The formula for $p_{>2, 1}(t, q)$ then follows from \Cref{lmm:29}.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  The formula for $p_{>3, 1, 1}(t, q)$ then follows from \Cref{lmm:29}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  together with \Cref{lmm:30}, \Cref{lmm:31} and \Cref{lmm:32}.
\end{proof}
...
\begin{proof}
  This follows from \Cref{lmm:33} by setting $t = 1$ and \cite[Theorem 4]{andrews_singular_2022} together with \Cref{lmm:29}.
\end{proof}
...
\begin{proof}
  The result follows immediately from \Cref{lmm:35}.
\end{proof}
...
\end{equation*}
We set $I = \ker(\gr^G(\pi_0))$, following the notation of \cite{andrews_singular_2022}, and we recall the definition of $K$ given in \eqref{eq:52}.
...
\begin{proof}
  ...
  \end{equation*}
  where $W$ is defined in \eqref{eq:50}.
  ...
  By \Cref{lmm:36} and \eqref{eq:53}, we have $a_{(-1)}\vachalf \in W \cap G^pM$ and also $\iota_0(\alpha^p(a)) = \alpha^p(a_{(-1)}\vachalf) \in K$, finishing the proof.
\end{proof}

\begin{remark}
  \label{rmk:48}
  The proof of \Cref{lmm:37} also works for other values of $c = c_{p, q}$ and $h = h_{m, n}$ as in \Cref{thr:44}.
\end{remark}
...
\end{equation*}
for $1 \le i \le p(n - k)$, $1 \le j \le p(n)$ and $k = 2, 3$, where $\lambda_1, \dots, \lambda_{p(n)}$ are the partitions of $n$ ordered in such a way that $L_{\lambda_1} > L_{\lambda_2} > \dots > L_{\lambda_{p(n)}}$, and $u_2, u_3$ are given by \eqref{eq:51}.
...
\begin{remark}
  \label{rmk:50}
  By construction and \Cref{rmk:49}, for each pivot $\lambda$ of $A^W_n$, we have $u^W_{\lambda} \in W$, $u^K_{\lambda} \in K$ and $\lm(u^K_{\lambda}) = u_{\lambda}$.
\end{remark}
...
By \Cref{lmm:37}, \Cref{rmk:50} and the fact that $K$ is a vertex Poisson submodule (not just a submodule), we have
\begin{equation*}
  K' \subseteq K.
\end{equation*}
...
\begin{proof}
  ...
  We pick $v \in K'$ such that $\lm(v) = u_{\eta}$ and some power product $p_{\tau} \in \mathbb{C}[L_{-2}, L_{-3}, \dots]$, for some partition $\tau$, such that $\lm((L_{-2})^k_{(0)}(p_{\tau}v)) = u_{\lambda}$ (this can be done because of \Cref{rmk:51} and the fact that $\lm$ is multiplicative).
...
\begin{proof}
  ...
  By \Cref{rmk:52}, $u_{[\eta, 1, \dots, 1]} \mid u_{\lambda}$, where we added $k$ ones to $\eta$, a contradiction.
  ...
  By \Cref{rmk:52}, if $\eta$ is equal to $[2]$, $[3, 3]$, $[r, r, r]$, $[r + 1, r, r]$ or $[r + 1, r + 1, r]$ for some $r \ge 3$, then $u_{\eta} \nmid u_{\lambda}$.
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  Therefore, by \Cref{lmm:34}, isomorphism \eqref{eq:54} and \Cref{prp:21}(i), we have
  \begin{equation*}
  ...
  By \Cref{lmm:38}, there would exist $u \in K' \cap F_N \subseteq K \cap F_N$ such that $\lm(u) = u_{\eta}$.
  ...
  We see that the family of bases given by \eqref{eq:55} satisfies the property that each basis is a subset of the next.
  This observation and \Cref{prp:19} conclude the proof of \Cref{thr:4}.
\end{proof}

\begin{proof}[Proof of \Cref{thr:3}]
  We recall that the basis of \Cref{thr:4} also gives a basis of $\gr^G(L)$.
  ...
  Therefore, $\ch_{\gr^G(L)}(t, q) = q^{1/2}p(t, q)$ and together with \Cref{lmm:33}, we conclude the proof of \Cref{thr:3}.
\end{proof}
...
\begin{example}[$\gr(U(\mathfrak{g}))$]
  ...
  By \Cref{exa:23}, $\gr(U(\mathfrak{g}))$ is canonically isomorphic to $S(\mathfrak{g})$.
...
\begin{remark}
  \label{rmk:53}
  We probably cannot prove the right Leibniz rule from the Left Leibniz rule for modules like in \Cref{lmm:40} because we do not have anticommutativity for modules.
...
If $f: R_1 \to R_2$ is a homomorphism of finitely generated algebras, then $Jf: JR_1 \to JR_2$ is defined by requiring that $Jf(\partial_1^n(\inc_1(x))) = \partial^n_2(\inc_2(f(x)))$ for $x \in R_1$ and $n \in \mathbb{N}$ (cf.\ \Cref{rmk:54}).
...
\begin{remark}
  ...
  As in \Cref{rmk:54}, we have $JR = (\inc(R))_{\partial}$.
...
We can extend uniquely the Hamiltonian $H$ to a Hamiltonian $H^{JR} \in \End(JR)$ because of \eqref{eq:56}.
...
\begin{proof}[Sketch of proof]
  ...
  \end{align*}
  is a solution to these equations.
  We derive the formula $p_{>2}(t, q) = f_{2, 2, 0, 0}(t, q)$ again from \Cref{lmm:29}.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \lambda &\mapsto [\lambda_1, \dots, \lambda_m, 1],
  \end{align*}
  from which, together with the proof of \Cref{lmm:42}, we get:
  \begin{align*}
    p_{>4, 1, 1}(t, q) &= tqp_{>4, 1}(t, q) = f_{6, 4, 2, 2}(t, q), \\
  ...
  \end{align*}
  The formula for $p_{>2, 1, 1}(t, q)$ then follows from \Cref{lmm:29}.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  from which, together with the proof of \Cref{lmm:41} and \Cref{lmm:29}, we get:
  \begin{align*}
  ...
  \end{align*}
  The formula for $p_{>3, 1, 1, 1}(t, q)$ then follows from \Cref{lmm:29}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  together with \Cref{lmm:41}, \Cref{lmm:42}, \Cref{lmm:43}, \Cref{lmm:44} and \Cref{lmm:29}.
\end{proof}
