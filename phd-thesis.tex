...
Formulas \eqref{eq:17} and \eqref{eq:18} are used to do explicit computations.
Many important vertex algebras are constructed from regular formal distribution Lie superalgebras, as the following theorem states.
...
\begin{theorem}
  ...
  \end{align*}
  Then $\mathcal{F}$ consists of fields and $(V, \vac, T, \mathcal{F})$ is a pre-vertex algebra.
  By the extension theorem, $V$ is a vertex algebra, denoted by $V(\mathfrak{g}, \mathcal{F}_0, T_0)$, and is explicitly given by
  \begin{equation}
    \label{eq:19}
  ...
  and $Y: V \to \mathcal{F}(V)$ is explicitly given by
  \begin{equation*}
    Y(a^{j_1}_{(-n_1 - 1)}\dots a^{j_s}_{(-n_s - 1)}\vac, z) = \frac{:\partial^{n_1}_za^{j_1}(z)\dots \partial^{n_s}_za^{j_s}(z):}{n_1!\dots n_s!}
  \end{equation*}
...
\begin{proof}
  Equality \eqref{eq:19} holds by the PBW theorem.
  ...
  By \eqref{eq:19}, these elements form a spanning set for $V$.
...
Usually we need to quotient the vertex algebra obtained with the previous theorem.

\begin{theorem}
  \label{thr:16}
  Let $(\mathfrak{g},\mathcal{F}_0,T_0)$ be a regular formal distribution Lie superalgebra and $\lambda:\mathfrak{h}\to \mathbb{C}$ a linear functional where $\mathfrak{h}$ is a subalgebra of $\mathfrak{g}$.
  Define the ideal
  \begin{equation*}
    I^\lambda=((a-\lambda(a))\vac\mid a\in \mathfrak{h}) \subseteq V(\mathfrak{g},F_0,T_0)
  \end{equation*}
  where we consider $a-\lambda(a)$ as an element of $U(\mathfrak{g})$.
  We get a vertex algebra
  \begin{equation*}
    V^\lambda(\mathfrak{g},F_0,T_0)=V(\mathfrak{g},F_0,T_0)/I^\lambda
  \end{equation*}
  and a vertex algebra homomorphism $\pi:V(\mathfrak{g},F_0,T_0)\to V^\lambda(\mathfrak{g},F_0,T_0)$ that satisfies $\pi(a\vac)=\lambda(a)\vac$ for all $a\in \mathfrak{h}$ and is universal with this property, i.e. if $f:V(\mathfrak{g},F_0,T_0)\to W$ is a vertex algebra homomorphism such that $f(a)\vac=\lambda(a)\vac$ for all $a\in \mathfrak{h}$, then there exist a unique vertex algebra homomorphism $\bar f:V^\lambda(\mathfrak{g},F_0,T_0)\to W$ such that the following diagram commutes
  \begin{equation*}
    \begin{tikzcd}
      {V(\mathfrak{g},F_0,T_0)} \arrow[rd, "f"] \arrow[r, "\pi"] & {V^\lambda(\mathfrak{g},F_0,T_0)} \arrow[d, "\bar f"] \\
      & W                                                    
    \end{tikzcd}
  \end{equation*}
  If $\mathfrak{h}\subseteq \mathfrak{g}_+$ is central and $T_0(\mathfrak{h})=0$ then
  \begin{equation*}
    I^\lambda=\sum_{a\in \mathfrak{h}}(a-\lambda(a))V(\mathfrak{g},F_0,T_0).
  \end{equation*}
  and
  \begin{equation*}
    V^\lambda(\mathfrak{g},F_0,T_0)\cong U(\mathfrak{g})\otimes_{U(\mathfrak{g}_-\oplus \mathfrak{h})}\mathbb{C}_\lambda
  \end{equation*}
  as an $\mathfrak{g}$ module, where $\mathbb{C}_\lambda=\mathbb{C}$ on which $\mathfrak{g}_-$ acts as zero and $a$ acts as $\lambda(a)$ for $a\in \mathfrak{h}$.
\end{theorem}
