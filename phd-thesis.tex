...
\begin{proposition}
  \label{prp:19}
  Let $M$ be a vector space, let $(M^p)_{p \in \mathbb{Z}}$ be a family of subspaces of $M$ satisfying properties \emph{(i)--(ii)} of \Cref{prp:18}, and let $(B^p)_{p \in \mathbb{N}}$ be a increasing family of sets satisfying $B^p \subseteq M^p$ for $p \in \mathbb{N}$.
