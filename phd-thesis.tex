...
\begin{proof}
  The proof of \Cref{thr:50} also works here.
\end{proof}
...
\begin{proof}
  The assertion follows from \Cref{lmm:26} and \Cref{thr:58}.
\end{proof}
...
If $f: R_1 \to R_2$ is a homomorphism of finitely generated algebras, then $Jf: JR_1 \to JR_2$ is defined by requiring that $Jf(\partial_1^n(\inc_1(x))) = \partial^n_2(\inc_2(f(x)))$ for $x \in R_1$ and $n \in \mathbb{N}$ (c.f.\ \Cref{rmk:53}).
...
\begin{remark}
  ...
  As in \Cref{rmk:53}, we have $JR = (\inc(R))_{\partial}$.
