...
In \Cref{sec:pbw-basis-l12}, we carry out the computations needed to obtain the PBW basis of $L(1/2, 1/2)$ and its refined character using results from the previous sections.
We also prove \Cref{thr:3} and \Cref{thr:4}.
In the appendices, we recall a couple results about almost commutative algebras needed to describe $\gr^G(\Vir^c)$, we show the SageMath \cite{sagemath} program used compute a Poisson submodule related to the exceptional partitions appearing in $R^{1/2}$ and briefly the case $L(1/2, 1/16)$.
...
In this thesis, we study the Ising model and its irreducible modules called \emph{Ising modules}.
...
Let $V$ be a vertex superalgebra.
An \emph{admissible} $V$-module is a $V$-module $M$ together with a grading $M = \bigoplus_{n \in \mathbb{N}}M(n)$ such that
\begin{equation*}
...
A \emph{homomorphism of admissible $V$-modules} is a homomorphism of modules that respects grading.
...
\end{equation*}
Give $U$ degree $0$ and extend the grading of $\Lie(V)$ to $M(U)$.
...
Let $V$ be an $\mathbb{N}$-graded conformal vertex algebra with conformal vector $\omega$, $(a^i)_{i \in I}$ a family of homogeneous strong generators of $V$ and let $M$ be an $h + \mathbb{N}$-graded $(V, \omega)$-module.
...
\begin{remark}
  ...
  However, if we do it like that, the characters wouldn't have good meanings because one can always shift the grading in admissible modules and also the term $q^h$ is lost.
\end{remark}
...
\begin{proof}[Proof of \Cref{thr:4}]
  ...
  We see that the family of bases given by \eqref{eq:41} satisfies the property that one basis is contained in the next one and this concludes the proof of \Cref{thr:4}.
\end{proof}
...
Let $\mathfrak{g}$ be a Lie algebra.
The \emph{tensor algebra} $T(\mathfrak{g}) = \bigoplus_{n \in \mathbb{N}}\mathfrak{g}^{\otimes n}$ of $\mathfrak{g}$ is an associative algebra with a unit $1$.
...
The \emph{symmetric algebra} $S(\mathfrak{g}) = T(\mathfrak{g})/(xy - yx \mid x, y \in \mathfrak{g})$ of $\mathfrak{g}$ is a commutative associative algebra with a unit $1$ and we have an inclusion $\mathfrak{g} \to S(\mathfrak{g})$ satisfying an universal property.
